...
if (file.exists(tfl_file)) {
  ...
  tfl_data <- dl_mdbzip("77062fa0-f409-4fe7-ae28-de8c933a59bd", verbose = T)
  tfl_file <- here::here("data", "tfl.Rdata")
...
```{r get_tfl_cl}
names(tfl_data$DRF_TAXON_CONSV_LISTINGS)
tfl_cons_listing <- tfl_data$DRF_TAXON_CONSV_LISTINGS %>%
  dplyr::transmute(
    ...
    date_listed = ifelse(DATE.LISTED == "",
    default_date,
    lubridate::parse_date_time2(DATE.LISTED, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    reasons = REASONS %>% as.character(),
    authority = AUTHORITY %>% as.character(),
    date_delisted = ifelse(DATE.DELISTED == "",
    default_date,
    lubridate::parse_date_time2(DATE.DELISTED, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    comments = COMMENTS %>% as.character(),
    ...
    recovery_plan_expiry_date = ifelse(RP.EXP.DATE == "",
    default_date,
    lubridate::parse_date_time2(RP.EXP.DATE, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    active_ind = ACTIVE.IND %>% as.character(),
    deactivated_date = ifelse(DEACTIVATED.DATE == "",
    default_date,
    lubridate::parse_date_time2(DEACTIVATED.DATE, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    reason_deactivated = REASON.DEACTIVATED %>% as.character(),
    created_by = CREATED.BY %>% as.character(),
    created_date = ifelse(CREATED.DATE == "",
    default_date,
    lubridate::parse_date_time2(CREATED.DATE, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    modified_by = MODIFIED.BY %>% as.character(),
    modified_date = ifelse(MODIFIED.DATE == "",
    default_date,
    lubridate::parse_date_time2(MODIFIED.DATE, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    file_last_updated = ifelse(FILE.LAST.UPDATED == "",
    default_date,
    lubridate::parse_date_time2(FILE.LAST.UPDATED, orders, tz = tz, cutoff_2000 = 19L)) %>%
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
...
tfl_hab_cond <- hab_cond %>%
  ...
  base::replace(., is.na(.), "") # 'occurrence_condition' cannot 
...
tfl_plant_count <- plant_count %>%
  ...
  dplyr::transmute(
    source = source,
    source_id = source_id,
    count_method = tsc_plant_count_method,
    counted_subject = tsc_counted_subject,
    plant_condition = tsc_Plant_condition,
    ...
    flowering_plants_percent = flowering_plants,
...
# some TAE missing? - doublecheck all source_ids in occ_observations exist in tfl_tae
setdiff(tfl_phys_sample$source_id,tfl_tae$source_id) # 22
setdiff(tfl_plant_count$source_id,tfl_tae$source_id) # 19
setdiff(tfl_hab_cond$source_id,tfl_tae$source_id) # 28
setdiff(tfl_area_ass$source_id,tfl_tae$source_id) # 10
setdiff(tfl_hab_comp$source_id,tfl_tae$source_id) # 36
```
...
# LONG RUNNING UPLOAD
tfl_tae %>% 
  chunk_post(serializer = "occ-taxon-points")
```
...
if (file.exists(tsc_occ_flora_datafile)) {
  ...
  tsc_taxa_flora <- "taxon" %>%
    wastdr::wastd_GET(api_url = prod, # can be removed?
...
tfl_tae_create <- tfl_tae %>% 
  dplyr::anti_join(tsc_tae, by = "source_id")

tfl_tae_update <- tfl_tae %>% 
  dplyr::right_join(tsc_tae, by = "source_id")
...
### tfl\_tcl conservation listings not in tsc\_tcl\_flora
...
### tsc\_tcl\_flora conservation listings not in tfl\_tcl
...
tsc_listing_cat_iucn1994 %>% 
  dplyr::anti_join(tfl_listing_cat_iucn1994, by = "source_id") %>% 
  rt
```
...
### tfl\_occ occurrences not in tfl\_tae
...
`r nrow(missing_occ_tfl_occ_flora_unique)` unique species. The occurrences 
listed were likely not migrated because they do not contain lat or long data 
which is a requirement for valid taxon area encounter in tsc.
...
### tfl\_occ names not in tfl\_tae
...
### tfl\_tae names not in tsc\_occ\_flora
