...
if (file.exists(tsc_occ_flora_datafile)) {
  ...
  # 44786 TFL
  tsc_tae_data <- wastdr::wastd_GET("occ-taxon-points", query = list(source=11))
  
  tsc_tae <- tsc_tae_data %>%
    #
    # version 1: json > geojson_sf
    # magrittr::extract2("data") %>%
    # geojsonio::as.json() %>%
    # geojsonsf::geojson_sf() %>%
    # wastdr::sf_as_tbl() %>% 
    # 
    # version 2: wastd_parse
    wastdr::wastd_parse()  %>%
    # turn taxon NameID into character for later joins
    dplyr::mutate(taxon = taxon %>% as.character())
...
length(unique(tsc_tae$source_id))
