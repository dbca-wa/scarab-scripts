...
```{r}
unique(tsc_flora_observations$source) == 11
...
tsc_plant_count_sum <-
  tsc_tae_observations_flora %>% make_tsc_occ_obs_summary("PlantCount")
...
Each table shows three example records for three species (source_id = 75917, 32829, 100069).
Any differences mean the data has not been migrated correctly. 
...
tsc_species_area_ass <- tsc_flora_observations %>% 
  ...
  select(c("source_id","tsc_survey_method"))
