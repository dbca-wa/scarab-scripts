...
tfl_plant_count_save <- here::here("data/tfl_plant_count.RData")
save(tfl_plant_count,
  file = tfl_plant_count_save
)
...
tfl_plant_count_res <- tfl_plant_count[2:3,] %>%
...
if (file.exists(tsc_extracted_flora)) {
  ...
  tsc_tae_observations_flora <- "occ-observation" %>% 
    wastdr::wastd_GET(query = list(source=11)) %>%
    wastdr::wastd_parse()
  
  tsc_tae_observations_flora %>% 
    as.tibble() %>% 
    tidyr::unpack(encounter)
...
tsc_phys_sample_sum <-
  tsc_tae_observations_flora %>% make_tsc_occ_obs_summary("PhysicalSample")
tsc_hab_cond_sum <-
  tsc_tae_observations_flora %>% make_tsc_occ_obs_summary("HabitatCondition")
tsc_area_ass_sum <-
  tsc_tae_observations_flora %>% make_tsc_occ_obs_summary("AreaAssessment")
tsc_hab_comp_sum <-
  tsc_tae_observations_flora %>% make_tsc_occ_obs_summary("HabitatComposition")
# tsc_plant_count_sum <- 
#   tsc_tae_observations_flora %>% make_tsc_cl_summary("PlantCount")
