...
tsc_tcl_flora <- wastdr::wastd_GET("taxon-conservationlisting") %>% 
  wastdr::parse_taxon_conservationlisting() %>% 
  dplyr::mutate(name_id = taxon) %>%
  dplyr::mutate(name_id = name_id %>% as.character()) %>%
...
if (file.exists(datafile)) {
  ...
  save(
    tsc_taxa_flora, # TSC taxonomic flora names
    tsc_tcl_flora, # TSC taxon conservation listings

    tfl_data, # entire TFL database
    tfl_tcl, # TFL conservation listings with TSC criteria IDs
    tfl_listing_cat_priority,
    tfl_listing_cat_epbc, # TFL cons listings split up by cons list
    tfl_listing_cat_iucn1994,
    tfl_listing_cat_iucn2001,
...
[Threatened Fauna Dataset](https://data.dpaw.wa.gov.au/dataset/threatened-and-priority-flora-database/) 
