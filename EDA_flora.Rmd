...
x <- tfl_tae[1,] %>% as.list() %>% 
  ...
  wastd_POST(serializer = "occ-taxon-points", 
             # comment in to test against local TSC:
             # api_url = dev, api_token = Sys.getenv("WASTDR_API_TOKEN_DEV"), 
             verbose = T)

# Upload all occurrences to TSC PROD
tfl_tae %>% chunk_post(serializer = "occ-taxon-points", verbose = T)
```

### Upload ObservationGroups

```{r, eval=FALSE}
tfl_phys_sample[1,] %>% wastd_bulk_post("occ-observation", verbose = TRUE)
...
datafile.flora <- here::here("data/flora_tae.RData")
tsc_tae_data <- wastdr::wastd_GET("occ-taxon-points") # 137k records

if (file.exists(datafile.flora)) {
  load(datafile.flora)
} else {
...
save(tsc_tae, tsc_taxa_flora, tsc_areas, file=datafile.flora, compress="xz")
}
```

```{r munge, message = FALSE, warning=FALSE}
datafile.flora <- here::here("data/flora_tae.RData")
if (fs::file_exists(datafile.flora)) load(datafile.flora)
...
The following table shows any conservation listings in TSC which are not in TFL. Becasue there are more taxa in TSC than TFL taxa with no source_id have also been removed.
...
# tfl_tae_tsc_occ_tally %>% reactable::reactable() ## Can't make this into a reactable without a way of changing as sfc feature to a tbl- Florian?
