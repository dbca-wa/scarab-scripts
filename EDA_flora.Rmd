...
they do not contain a name_id and/or a location.
...
`r nrow(missing_occ_tfl_occ_flora_unique)` unique species. The occurrences listed were likely not migrated because
...
missing_taxa_tsc_unique <- missing_taxa_tfl_tae %>%
...
The table below is a tally of mismatching name_id's and confirms the missing
occurrences identified above.

```{r show_tally_tfl_occ_tae_flora}
...
tfl_occ_tfl_tae_tally <- tfl_occ_tally %>%
  dplyr::rename(occ = n) %>%
  left_join(tfl_tae_tally, by = c("name_id" = "taxon")) %>%
  dplyr::rename(tae = n) %>%
  filter(occ != tae)

tfl_occ_tfl_tae_tally %>% rt 
...
There are `r nrow(missing_taxa_tsc)` taxon records missing from `tsc_taxa_flora` when
compared to `tfa_tae`. These records are attributed to
`r nrow(missing_taxa_tsc_unique)` unique species.
