...
if (file.exists(tsc_cl_flora_datafile)) {
  ...
  tsc_tcl_flora <- "taxon-conservationlisting" %>% 
    ...
    dplyr::inner_join(tsc_taxa_flora, by = "taxon") # If we use inner_join here 
  # the date comparisions with tfl below match, if we use right_join, which was 
  # here before, we get mismatches.
...
Currently, all Flora occurrences are georeferenced only with point
...
manual mapping can be updated and the correct criteria assigned for all species.
...
#  No. It happens if you right_join rather than inner_join tsc_taxa_flora to tsc_tcl_flora. I have 
#  changed this above to be an inner_join, is there any reason I shouldn't?
```
...
tfl_listing_cat_iucn2001 %>%
  dplyr::anti_join(tsc_listing_cat_iucn2012, by = c("source_id")) %>% 
  rt
```
...
`tsc_tae` when compared to `tfl_tae`. These missing occurrences come from
