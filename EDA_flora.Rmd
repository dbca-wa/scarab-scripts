...
tfl_plant_count <- plant_count %>%
  ...
  dplyr::select(-plant_count_method,-Counted_subject,-Plant_condition,-tsc_criteria.x,-tsc_criteria.y,-tsc_criteria) %>%
...
tfl_soiltype: Three of these criteria cound not be mapped because and we don't know what they are.
tfl_sampdest (sample destination): This has manually mapped to a generic field for its location and the specific location added to the smaple label field as free text. We have done this because, combined with Fauna, there are > 100 unique locations for sample destination, which would make a very long drop down menu.
