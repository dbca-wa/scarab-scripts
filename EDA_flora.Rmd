...
if (file.exists(tfl_file)) {
  ...
  save(tfl_data, snapshot_last_updated, file = tfl_file)
}
...
```{r savepoint_tfl}
tfl_datafile <- here::here("data/tfl_all.RData")
save(
  act_fenc,
  act_rdmk,
  all_crit_map,
...
if (file.exists(datafile_tsc_flora)) {fs::file_delete(datafile_tsc_flora)}
```
...
# LONG RUNNING UPLOAD
tfl_tae_res <- tfl_tae[1:10000,] %>% 
  chunk_post(serializer = "occ-taxon-points", chunksize = 100) #44791
```
...
# Bulk upload
# LONG RUNNING UPLOAD
tfl_phys_sample_res <- tfl_phys_sample[1:10000,] %>%
  wastd_occ_obs_post(obstype="PhysicalSample") #23632
tfl_hab_cond_res <- tfl_hab_cond[1:10000,] %>%
  wastd_occ_obs_post(obstype="HabitatCondition") #22181
tfl_area_ass_res <- tfl_area_ass[1:9899,] %>%
  wastd_occ_obs_post(obstype="AreaAssessment") #9899
tfl_plant_count_res <- tfl_plant_count[1:10000,] %>%
  wastd_occ_obs_post(obstype="PlantCount") #38624
tfl_hab_comp_res <- tfl_hab_comp[1:10000,] %>%
  wastd_occ_obs_post(obstype="HabitatComposition") #40996

# setdiff(tfl_phys_sample$source_id,tfl_tae$source_id)
...
if (file.exists(tsc_cl_flora_datafile)) {
  ...
  # 33382 results shown at
  # https://tsc.dbca.wa.gov.au/api/1/taxon/?paraphyletic_groups__icontains=21
...
if (file.exists(tsc_occ_flora_datafile)) {
  ...
  tsc_tae_data <- wastdr::wastd_GET("occ-taxon-points", query = list(source=11)) # 44786
  ...
  tsc_tae3 <- tsc_tae_data %>%
    wastdr::wastd_parse()  %>% 
    dplyr::mutate(taxon = taxon %>% as.character())
  
  # Already run in prev chunk
  # tsc_taxa_flora <- "taxon" %>%
  ...
  tsc_occ_flora <- tsc_tae %>%
    ...
    # dplyr::mutate(taxon = taxon %>% as.character()) %>%
    # dplyr::inner_join(tsc_taxa_flora, by = "taxon") # NO - tsc_taxa_flora contains fauna taxa
    dplyr::filter(source == 11)
...
# Check tsc_occ_flora correct
tsc_occ_flora_no <- tsc_tae %>%
  dplyr::filter(source == 11)

# All sources in tsc_tae: 10 11
unique(tsc_tae$source) == c(10,11)

# FM RSS,    R 4.0.0, dplyr 0.8.5: 106 140 RFA + 44 847 TFL = 150 987 TSC TAE
# tsc_tae3 106748 10451
# tsc_tae2 same
# tsc_tae  same
tsc_tae %>% dplyr::filter(source == 10) %>% nrow
tsc_tae %>% dplyr::filter(source == 11) %>% nrow # == tsc_occ_flora
unique(tsc_occ_flora$source) == 11
tsc_occ_flora %>% dplyr::filter(source == 10) %>% nrow # 0 Fauna Occ
tsc_occ_flora %>% dplyr::filter(source == 11) %>% nrow # 44 847 Fauna Occ

# However, different number of unique source IDs
# FM RSS,    R 4.0.0, dplyr 0.8.5: 42 713 on Mon 08 Jun
length(unique(tsc_occ_flora$source_id))
tsc_tae_tally <- tsc_tae %>% 
  group_by(source, source_id) %>% 
  dplyr::tally() %>% 
  dplyr::ungroup() %>% 
  dplyr::arrange(-n)
View(head(tsc_tae_tally))

# What is the difference between tsc_tae and tsc_tae2 and tsc_tae3?
# https://bookdown.org/Maxine/r4ds/comparing-two-data-frames-tibbles.html
testthat::expect_equal(tsc_tae2, tsc_tae3)
dplyr::all_equal(tsc_tae2, dplyr::select(tsc_tae3, -id, -type, -geometry))


## Check for duplicates
tsc_tae_flora_dup <- tsc_tae %>%
  dplyr::filter(source == 11) %>% 
  dplyr::filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])]))

tsc_occ_flora_dup <- tsc_occ_flora %>%
  filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])]))

#### Identify missing tae in tsc_tae
#2448 # 2211 # 2293
tfl_tae_create <- tfl_tae %>% dplyr::anti_join(tsc_occ_flora, by = "source_id")

 #42343 # 42580 # 42498
tfl_tae_update <- tfl_tae %>% dplyr::semi_join(tsc_occ_flora, by = "source_id")

## Upload the missing tfa_tae to tsc_tae
# tfl_tae_create %>% chunk_post(serializer = "occ-taxon-points", chunksize = 100)
```
...
Because there are more taxa in TSC than TFL, taxa with no source\_id have also
been removed.
...
Below are occurrence observations which have not been migrated because they do 
not have a corresponding TAE for them to be assigned to. 
...
The above need to be reviewed and a decision made on whether the information 
should be migrated despite not having a TAE to assign it to.
