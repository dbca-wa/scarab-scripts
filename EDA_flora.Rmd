...
tsc_tcl_flora <- tsc_tcl %>% 
  dplyr::inner_join(tsc_taxa,by = "name_id") %>% glimpse()
```
...
```{r tfa_summary}
tfl_tcl_flora <- tfl_tcl %>%
  dplyr::mutate(source_id = source_id %>% as.character())

tfl_tcl_unique_taxa <- tfl_tcl_flora$name_id %>%
  unique() %>%
  length() 
tsc_tcl_unique_taxa <- tfl_tcl_flora$name_id %>%
  unique() %>%
  length()
```
...
TODO remove animal tcl from this analysis.
...
```{r}
tfl_tcl %>% dplyr::filter(name_id == 0)
```
