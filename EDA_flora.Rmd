...
# pop_forms is the paper form, not the shape of the population!
...
## Occurrences
```{r load_tfl_occ, message=FALSE, warning=FALSE}
...
Conservation criteria Data from TSC is extracted through the TSC API and transformed into a useable format for manual mapping to tfl.
...
## Conservation listings
```{r tfl_cons_listing_subsetting, message=FALSE}
...
```{r tfl_cons_listing_upload, eval=F, message=FALSE}
...
```{r, eval=FALSE}
tfl_phys_sample[1,] %>% wastd_bulk_post("occ-observation")

tfl_phys_sample %>% wastd_bulk_post("occ-observation")

tfl_hab_cond %>% wastd_bulk_post("occ-observation")

tfl_area_ass %>% wastd_bulk_post("occ-observation")

tfl_plant_count %>% wastd_bulk_post("occ-observation")

tfl_hab_comp %>% wastd_bulk_post("occ-observation")
```
...
```{r extract_tsc_occ, eval = FALSE}
tsc_occ_datafile <- here::here("data/flora_tae.RData")

if (file.exists(tsc_occ_datafile)) {
  load(tsc_occ_datafile)
} else {
...
tsc_occ_datafile <- here::here("data/flora_tae.RData")
if (fs::file_exists(tsc_occ_datafile))
  load(tsc_occ_datafile)
...
Becasue there are more taxa in TSC than TFL taxa with no source_id have also been removed.
...
tfl_listing_cat_iucn2012 %>%
  dplyr::anti_join(
    ...
    by = c("source_id", "effective_from", "status", "effective_to")
  ) %>% rt
```
IUCN 2001
```{r}
...
NameID, and `r nrow(tfl_occ_no_popid)` distinct scientific flora names without a PopID.
...
TODO Which datum do we want to use? WGS84 and GDA94 are designed to be <1m different. 
GDA94 is designed to be comparable with WGS84 GPS data. 
As GDA94 is the most frequently used datum in the dataset, should we convert all others to GDA94?
...
```{r name mismatches tfl_tae to tsc_tae}
...
```{r name mismatches tfl_tae to tsc_occ_flora}
...
```{r missing_occurrences tfl_tae to tsc_tae}
...
```{r missing_occurrences tfl_tae to tsc_occ_flora}
