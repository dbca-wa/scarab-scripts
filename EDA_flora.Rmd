...
This workbook summarises the legacy data migration and QA for the Threatened and
Priority Flora database (TFL).

![](https://data.dpaw.wa.gov.au/dataset/79e90d7b-33a3-4aa0-8634-8a12033eb21d/resource/98588a4e-b3af-452b-bbf4-f63ca7697752/download/tsc-progress.jpg){width="100%"}
...
Data from the Threatened Flora Database (TFL) is downloaded from the [TFL
dataset](https://data.dpaw.wa.gov.au/dataset/threatened-and-priority-flora-database)
...
tsc_cl_flora_datafile <- here::here("data/tsc_cl_flora.RData")
tsc_occ_flora_datafile <- here::here("data/tsc_tae_flora.RData")
...
```{r make_tfl_occ, message=FALSE, warning=FALSE}
...
```{r load_tfl_tcl, eval=F}
# LONG RUNNING UPLOAD
tfl_listing_cat_priority %>% wastdr::wastd_POST("taxon-conservationlisting")
tfl_listing_cat_epbc %>% wastdr::wastd_POST("taxon-conservationlisting")
tfl_listing_cat_iucn1994 %>% wastdr::wastd_POST("taxon-conservationlisting")
tfl_listing_cat_iucn2001 %>% wastdr::wastd_POST("taxon-conservationlisting")
...
recorded flora occurrences, including fossil records. For further details on the
methodology behind the movement of EOO data, please follow the link to the
workbook.

### TSC upload example for EOOs

```{r load_one_taxon_eoo, eval=F}
...
# Example: one record
tfl_phys_sample[1,] %>% wastdr::wastd_bulk_post("occ-observation")

# Bulk upload
# LONG RUNNING UPLOAD
tfl_phys_sample %>% wastdr::wastd_bulk_post("occ-observation")
tfl_hab_cond %>% wastdr::wastd_bulk_post("occ-observation")
tfl_area_ass %>% wastdr::wastd_bulk_post("occ-observation")
tfl_plant_count %>% wastdr::wastd_bulk_post("occ-observation")
tfl_hab_comp %>% wastdr::wastd_bulk_post("occ-observation")
```
...
if (file.exists(tsc_cl_flora_datafile)) {
  load(tsc_cl_flora_datafile)
} else {
  ...
  save(tsc_taxa_flora, tsc_tcl_flora, file = tsc_cl_flora_datafile)
}
```
...
We then get TSC Areas and split them into DBCA Regions and Districts. Caveat:
Regions and Districts to not comprehensively cover WA.
...
```{r get_tsc_occ}
if (file.exists(tsc_occ_flora_datafile)) {
  load(tsc_occ_flora_datafile)
} else {
...
unique taxa. TSC has `r nrow(tsc_tcl_flora)` flora conservation listings of
...
Some TFL cons listings have a name\_id of 0. These are phrase names and require
...
in the `list_code` / `category_code`. These need to be corrected before the manual mapping
can be updated and the correct criteria assigned for all species.
...
cons_cat_summary %>% rt ## ARE THESE DIFFERENCES FROM USE OF CATEGORY_CODE AND NOT CATEGORY_CODE_WA?
```
...
tsc_listing_cat_iucn2001 %>% 
  dplyr::anti_join(tfl_listing_cat_iucn2001, by = "source_id") %>% 
  DT::datatable()
```
...
-   `r tfl_occ %>% dplyr::filter(created_date == "") %>% nrow()` records lack an
...
```{r show_tfl_occ_missing_species_name}
...
Soil type: Three of these criteria could not be mapped because and we don't know
what they are.
...
Any records in the following table indicate occurrences in tfl that were not
migrated into TSC.
...
There are `r nrow(missing_occ_tfl_tae)` occurrences missing from tfl\_tae when
compared to tfl\_occ. The occurrences listed were likely not migrated because
...
tfl_occ_tally %>%
  dplyr::rename(tfl = n) %>% 
  left_join(tfl_tae_tally,by = "source_id") %>% 
  dplyr::rename(tae = n) %>% 
  filter(tfl != tae) %>% rt
```
...
Any records in the following table indicate taxon in TFL that were not migrated
into TSC.

#### Name mismatches tfl\_tae to tsc\_tae
...
```{r show_name_mismatches_tfl_tae_to_tsc_occ_flora}
...
missing_taxa_occ_flora_unique %>% rt
```

There are `r nrow(missing_taxa_occ_flora)` taxon records missing from
`tsc_occ_flora` when compared to `tfl_tae`. These records are attributed to
`r nrow(missing_taxa_occ_flora_unique)` unique species.

#### Occurrence mismatches `tfl_tae` to `tsc_tae`

```{r show_missing_occurrences_tfl_tae_to_tsc_tae}
missing_occ_tae_tsc <- tfl_tae %>%
  dplyr::anti_join(tsc_tae, by = "source_id")

missing_occ_tae_tsc %>% rt
...
missing_occ_tae_tsc_unique <- missing_occ_tae_tsc %>%
  dplyr::select(taxon) %>%
  distinct() %>%
  dplyr::left_join(tsc_taxa_flora, by = "taxon") #705
```

There are `r nrow(missing_occ_tae_tsc)` unique occurrences missing from `tsc_tae`
when compared to `tfl_tae`. These missing occurrences come from
...
# tfl_tsc_tae_tally %>% rt
# ## Can't make this into a reactable without a way of changing as sfc feature to a tbl- Florian?
```

#### Occurrence mismatches tfl\_tae to tsc\_occ\_flora

```{r show_missing_occurrences_tfl_tae_to_tsc_occ_flora}
...
`tsc_occ_flora` when compared to `tfl_tae`. These missing occurrences come from
`r nrow(missing_occ_tfl_occ_flora_unique)` unique species.

The table below is a tally of mismatching occurrences and confirms the missing
occurrences identified above.

```{r tally_tfl_tae_occ_flora}
tfl_tae_tally <- tfl_tae %>%
  dplyr::group_by(source_id) %>%
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>%
...
The [compiled version of this workbook]() is uploaded to the [Threatened Flora
Dataset](https://data.dpaw.wa.gov.au/dataset/threatened-and-priority-flora-database/)
...
# upload_to_ckan(flora_cons_listing, "TPFL Conservation Listings", 
