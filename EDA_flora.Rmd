...
# LONG RUNNING UPLOAD
tfl_tae %>% 
  chunk_post(serializer = "occ-taxon-points", chunksize = 100)
```

### Upload Observation Groups
...
# Bulk upload
# LONG RUNNING UPLOAD
tfl_phys_sample_res <- tfl_phys_sample[23591:23632,] %>%
  wastd_occ_obs_post(obstype="PhysicalSample")
x <- tfl_phys_sample[23591:23632,]
tfl_hab_cond_res <- tfl_hab_cond[22091:22181,] %>%
  wastd_occ_obs_post(obstype="HabitatCondition")
...
```{r show_duplicates_and_missing_tae}
## Check for duplicates
tsc_flora_duplicates <- here::here("data/tsc_tae_flora_duplicates.RData")

tsc_tae_dup <- tsc_tae %>%
  filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])]))

tsc_tae_dup_2 <- tsc_tae %>% group_by(source_id) %>% filter(n()>1)
...
save(tsc_tae_dup,
     tsc_occ_flora_dup,
     file = tsc_flora_duplicates,
       compress = "xz")

#### Identify missing tae in tsc\_tae
tfl_tae_create <- tfl_tae %>% 
  dplyr::anti_join(tsc_occ_flora, by = "source_id") #2448

tfl_tae_update <- tfl_tae %>% 
  dplyr::semi_join(tsc_occ_flora, by = "source_id") #42343

## Upload the missing tfa_tae to tsc_tae
tfl_tae_create_res <- tfl_tae_create %>% chunk_post(serializer = "occ-taxon-points", chunksize = 100)
```
