...
tfl_tae %>% dplyr::group_by(taxon, name) %>% dplyr::tally() %>% dplyr::filter(n>2) %>% dplyr::arrange(-n) %>% reactable::reactable(filterable = TRUE)
```
...
tfl_listing_cat_iucn2012 %>% anti_join(tsc_listing_cat_iucn2012,by = "taxon") %>% 
...
tfl_listing_cat_iucn2001 %>% anti_join(tsc_listing_cat_iucn2001,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
IUCN 1994
```{r}
...
tfl_listing_cat_iucn1994 %>% anti_join(tsc_listing_cat_iucn1994,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
Priority
```{r}
...
tfl_listing_cat_priority %>% anti_join(tsc_listing_cat_priority,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
EPBC
```{r}
...
tsc_listing_cat_epbc <- tsc_tcl_flora %>%
  dplyr::filter(str_detect(category_cache,"EPBC")) ## what is the category_cache made from? List_code or list_code_wa?

tfl_listing_cat_epbc %>% anti_join(tsc_listing_cat_epbc,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
...
```{r }
#### IUCN 2012
tsc_listing_cat_iucn2012 %>% anti_join(tfl_listing_cat_iucn2012,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
IUCN 2001
```{r}
#### IUCN 2001
tsc_listing_cat_iucn2001 %>% anti_join(tfl_listing_cat_iucn2001,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
IUCN 1994
```{r}
#### IUCN 1994
tsc_listing_cat_iucn1994 %>% anti_join(tfl_listing_cat_iucn1994,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
IUCN Priority
```{r}
#### Priority
tsc_listing_cat_priority %>% anti_join(tfl_listing_cat_priority,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
EPBC
```{r}
#### EPBC
tsc_listing_cat_epbc %>% anti_join(tfl_listing_cat_epbc,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
...
### Flora occurrences missing a NameID
...
There are `r nrow(tfl_occ_no_nameid)` distinct scientific flora names without NameID.
...
tfl_occ_no_latlng %>% head %>% reactable::reactable()
```
...
```{r tally_occ,eval=FALSE}
tfl_tally <- tfl_occ %>%
  dplyr::group_by(name_id, species_name) %>%
  dplyr::tally() %>%
  dplyr::arrange(desc(n))

tfl_tally %>% head(n = 20) %>% reactable::reactable()
```

There are ???? flora taxa with occurrences.

### TSC occurences not in TFA
