...
names_tfl <- as.data.frame(names(tfl_data))
...
tfl_fire_hist <- tfl_occ %>%
  ...
  transmute(source = source,
    source_id = source_id,
    fire_intensity = fire_intensity,
    fire_date = glue::glue("{day}/{month}/{fire_year}", .na = "NA") %>% as.character) %>% 
  mutate(fire_date = case_when(is.na(fire_intensity) ~ "NA",
  TRUE ~ as.character(fire_date))) %>% glimpse()
...
tfl_tcl <- tfl_cons_listing %>% 
  left_join(tfl_conscrit,by=c("list_code_wa","criteria_code_wa")) %>% glimpse
```
...
There are `r nrow(datum_blank)` distinct fauna records with missing datum.
...
```
Sample destination was manually mapped to a generic field for its location and the specific location added to the smaple label field as free text. We have done this because, combined with Fauna, there are > 100 unique locations for sample destination, which would make a very long drop down menu.
...
tfl_occ_tally <- tfl_occ %>%
  ...
  dplyr::arrange(desc(n)) %>% 
  ungroup() %>% 
  dplyr::rename(taxon = name_id)

tfl_occ_tally_top <- tfl_occ_tally %>% 
  top_n(20,n)

tfl_occ_tally_btm <- tfl_occ_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) 
...
tae_tally_btm <- tae_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) # There are actually 301 species with an occurrence of 1

missing_species <- tae_tally %>% anti_join(tfl_occ_tally,by = "taxon")

tfl_occ_tally %>%
  dplyr::rename(tfl = n) %>% 
  left_join(tae_tally,by = "taxon") %>% 
  dplyr::rename(tae = n) %>% 
  filter(tfl != tae) %>% reactable::reactable()
```
There are `r nrow(missing_species)` species missing from tfl_tae when compared to tfl_occ. 

### TAE occurrences not in TSC

```{r tally_tsc_occ,eval=FALSE}
tsc_occ_tally <- tsc_occ_flora %>%
  dplyr::group_by(taxon) %>% 
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>% 
  ungroup()
# tsc_occ_tally %>% reactable::reactable()

tsc_occ_tally_top <- tsc_occ_tally %>% 
  top_n(20,n)

tsc_occ_tally_btm <- tsc_occ_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) 

missing_species_tae_tsc <- tsc_occ_tally %>% anti_join(tae_tally,by = "taxon")

missing_occurrences <- tae_tally %>%
  dplyr::rename(tae = n) %>% 
  left_join(tsc_occ_tally,by = "taxon") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tae != tsc) #%>% reactable::reactable()
