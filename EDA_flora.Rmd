...
plant_count <- tfl_occ %>%
  ...
  # dplyr::mutate(plant_count_method = 
  #                 case_when(plant_count_method == "ACT_CLMP" ~ "actual-count-clumps",
  #                           plant_count_method == "ACT_IND" ~ "actual-count-individuals",
  #                           plant_count_method == "EXT_VIQD" ~ "area-population-m2",
  #                           plant_count_method == "ESTMT" ~ "estimate",
  #                           plant_count_method == "EXT_LOOK" ~ "extrapolated-count-look-around",
  #                           plant_count_method == "EXT_GRQD" ~ "extrapolated-count-quadrat-based",
  #                           plant_count_method == "EXT_TRNS" ~ "extrapolated-count-transect-based",
  #                           plant_count_method == "EXT_TRAV" ~ "	extrapolated-count-traverse",
  #                           plant_count_method == "PART_CNT" ~ "partial-count-spatially-defined-subset-population",
  #                           plant_count_method == "UNKNOWN" ~ "unknown")) %>%
  # dplyr::filter(
  #   plant_count_method != "" | 
  #     counted_subject != "" | 
  #     plant_condition != "" | 
...
count_method_lookup <- plant_count %>% 
  dplyr::transmute(plant_count_method = plant_count_method) %>% 
  dplyr::distinct()

lookups <- tfl_data$DRF_LOOKUP_TERMS %>% 
  dplyr::filter(DOMAIN.CODE =="CNT_MTHD")
...
tfl_tae <- tfl_occ %>%
  ...
  dplyr::transmute(
    ...
    status = form_status_code %>% as.character(),
...
transformed into a usable format for manual mapping to TFL.
...
tfl_plant_count <- plant_count %>%
  ...
  dplyr::transmute(
    ...
    count_subject = tsc_counted_subject,
...
tfl_listing_cat_priority_res <- tfl_listing_cat_priority %>% 
  tsc_chunk_post("taxon-conservationlisting", chunksize = 100, verbose = TRUE)
tfl_listing_cat_epbc_res <- tfl_listing_cat_epbc %>% 
  tsc_POST("taxon-conservationlisting", verbose = TRUE)
...
# tfl_cl_iucn2012 %>% tsc_POST("taxon-conservationlisting")
...
tfl_eoo %>% dplyr::select(-data, -eoo_sfc) %>% tsc_POST("taxon")
```
...
# LONG RUNNING UPLOAD
tfl_tae %>% tsc_chunk_post(serializer = "occ-taxon-points")
```
...
#   tsc_occ_obs_post(obstype="HabitatCondition")
# tfl_area_ass_res <- tfl_area_ass[1,] %>%
#   tsc_occ_obs_post(obstype="AreaAssessment")
tfl_plant_count_res <- tfl_plant_count[1,] %>%
  tsc_occ_obs_post(obstype="PlantCount", api_url = uat)
# tfl_hab_comp_res <- tfl_hab_comp[1,] %>%
#   tsc_occ_obs_post(obstype="HabitatComposition")

# Bulk upload
# LONG RUNNING UPLOAD
tfl_phys_sample_res <- tfl_phys_sample %>%
  tsc_occ_obs_post(obstype="PhysicalSample", chunksize = 1000) #23632
tfl_hab_cond_res <- tfl_hab_cond %>%
  tsc_occ_obs_post(obstype="HabitatCondition", chunksize = 1000) #22181
tfl_area_ass_res <- tfl_area_ass %>%
  tsc_occ_obs_post(obstype="AreaAssessment", chunksize = 1000) #9899
tfl_plant_count_res <- tfl_plant_count[1:1000,] %>%
  tsc_occ_obs_post(obstype="PlantCount", api_url = uat, chunksize = 1000) #38624
tfl_hab_comp_res <- tfl_hab_comp %>%
  tsc_occ_obs_post(obstype="HabitatComposition", chunksize = 1000) #40996
...
if (file.exists(tsc_extracted_flora)) {
  ...
  tsc_taxa_flora <- "taxon" %>%
    tsc_GET(query = list(paraphyletic_groups = 21)) %>%
    tsc_parse() %>%
  ...
  tsc_flora_observations <- tsc_tae_observations %>% 
...
# tfl_tae_create %>% tsc_chunk_post(serializer = "occ-taxon-points")
```
...
tsc_corner_dates <- tsc_tcl_flora %>%
  summarise(
    ...
    review_due_on_min = min(review_due, na.rm = T),
    review_due_on_max = max(review_due, na.rm = T)
  )
...
tfl_corner_dates <- tfl_tcl %>%
  summarise(
    ...
    review_due_on_min = min(recovery_plan_expiry_date, na.rm = T),
    review_due_on_max = max(recovery_plan_expiry_date, na.rm = T)
  )
