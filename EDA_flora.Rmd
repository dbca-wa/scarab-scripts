...
lookups <-	tfl_data$DRF_LOOKUP_DOMAINS
...
fire_hist <- tfl_occ %>%
  ...
  dplyr::transmute(
    ...
    Date_of_last_fire = glue::glue(
      "{fire_year}", "{fire_season}")) # modify this to fit card requirements

fire_hist %>%
  dplyr::select(fire_intensity) %>%
  distinct() %>% 
  readr::write_csv(path = here::here("data", "tfl_fire_intensity.csv"))

fire_hist %>%
  dplyr::select(fire_year,fire_season) %>%
  distinct() %>% 
  readr::write_csv(path = here::here("data", "tfl_fire_date.csv"))
...
tfl_det_meth <- here::here("tfl_detmeth_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tfl_sec_sign <- here::here("tfl_secsign_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tfl_repmap <- here::here("tfl_repmat_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tfl_anim_obs <- anim_obs %>%
  left_join(tfl_confidence, by = c("species_id_confidence")) %>% 
  left_join(tfl_det_meth, by = c("detection_method")) %>%
  left_join(tfl_sec_sign, by = c("secondary_signs")) %>%
  left_join(tfl_repmap, by = c("reproductive_maturity")) %>%
...
tfl_samptype <- here::here("tfl_samptype_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tfl_sampdest <- here::here("tfl_sampdest_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tfl_phys_sample <- phys_sample %>%
  left_join(tfl_samptype, by = c("sample_type")) %>% 
...
tfl_tally <- tfl_occ %>%
  ...
  dplyr::arrange(desc(n)) %>% 
  ungroup()
tfl_tally %>% reactable::reactable()

tfl_tally_top <- tfl_tally %>% 
  top_n(20,n)

tfl_tally_btm <- tfl_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) # There are actually 301 species with an occurance of 1
```

There are `r nrow(tfl_tally)` fauna taxa with occurrences in tfl.

TODO Once we have data out of TSC compare tally of occurances for top 20 and bottom 20 species

### TSC occurences not in tfl
...
missing_species <- tae_tally %>% anti_join(tfl_tally,by = "name_id")
