...
```{r tfa_cl, eval=T}
tfa_cl %>% 
    select(list_code, status_code, status) %>% 
    rename(category_code = status_code, category_label=status) %>% 
...
```{r tfa_used, eval=T}
tfa_gaz %>% dplyr::select(list_code, status_code) %>% unique() %>% dplyr::arrange(list_code) %>% kable(.)
```
...
```{r tfl_cl, eval=T}
tfl_gaz_list %>% 
    dplyr::transmute(list_code=list_code, category_code = status_code) %>% 
    unique() %>% 
    dplyr::arrange(list_code, category_code) %>%
    kable(.)
```
