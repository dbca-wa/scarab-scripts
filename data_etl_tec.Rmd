...
Transform communities into TSC shape.
```{r com_tsc_tf}
com_tsc <- communities %>% 
  dplyr::transmute(
    source = 12, # TEC
    ...
    description = glue::glue(
      "Original area: {com_original_area}\n",
      "Area accuracy: {com_area_accuracy}\n",
      "Former range: {former_range}\n",
      "Range decline: {range_decline}\n",
      "Occurrence decline: {occ_decline}"
    )
  )
```

Load communities into TSC.
```{r com_tsc_load, eval=F}
com_tsc %>% wastdr::wastd_POST("community", api_url = prod)
```
...
TEC cons linsting categories closure date is the effective from date
of next (if any) cons listing within same scope.
`tec_cons_listing_categories$cat_endorsed_code` is approval life cycle stage.
`tec_cons_listing_categories$cat_comment` 
...
```{r join_tsc_conscrit_listing_to_tec_conscrit}
chr2int <- . %>% 
      stringr::str_split(",") %>% 
...
tec_cl_crit_w_tsc<-tec_cons_listing_criteria %>% 
  left_join(tec_conscrit,by=c("criterion_code"))
```
...
tsc_crit_code <- tec_cl_crit_w_tsc %>%
  ...
  dplyr::mutate(tsc_criteria = tsc_criteria %>% chr2int())
...
Upload to TSC is easy (although making it easy wasn't).
