...
tec_cae <- tibble::tibble(
    ...
    code=tec_boundaries$FIRST_S_ID %||% tec_boundaries$BDY_ID,
    name=tec_boundaries$FIRST_S_ID %||% tec_boundaries$BDY_ID,
    description = ifelse(
        is.na(tec_boundaries$FIRST_S_ID),
        glue::glue("Boundary ID {tec_boundaries$BDY_ID} named {tec_boundaries$BDY_ID} for lack of sites within."),
        glue::glue("Boundary ID {tec_boundaries$BDY_ID} named {tec_boundaries$FIRST_S_ID} after first site observed within.")
    ),
...
tec_cae %>% filter(community == "SCP20a")
tec_cae %>% 
    # filter(community == "SCP20a") %>% 
    chunk_post(serializer = "occ-community-areas", api_url = prod, verbose = T)
```

### Boundaries from raster
