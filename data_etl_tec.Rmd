...
The data model of conservation listings (gazettals) requires clarification.
...
### Cons listing TSC ETL
Transform TEC data into TSC data.

```{r tec_listing_tsc_transform}

# https://tsc.dbca.wa.gov.au/api/1/communitygazettal/
# 
# "source": 3, # TEC
# "source_id": "1", # tec$CATEGORIES row ID
# "scope": 0,  # WA
# "status": 80, # listed
# "proposed_on": null,
# "effective_from": "2008-12-08T12:00:00+09:00",
# "effective_to": null,
# "last_reviewed_on": null,
# "review_due": null, # cat_review_date
# "comments": "Transferred as-is from legacy TEC database.",
# "community": "Whicher Scarp A1", # com_label
# "category": [35] # WAPEC P1
...
tsc_conscat <- "conservationcategory" %>% 
  ...
  dplyr::filter(conservation_list %in% c(11,12))

# TODO join by scope as well
tec_listing_cat <- tec_cons_listing_categories %>% 
  dplyr::left_join(tsc_conscat, by="category_code") %>% 
  dplyr::transmute(
        source = 3,
        ...
        last_reviewed_on = cat_review_date,
        community = com_label,
...
Upload to TSC. Some TEC names might have changed, need to update commuity names again!

```{r tec_listing_tsc_upload, eval=F}
# typo: Broomhill System > Broomehill System
tec_listing_cat %>% 
  dplyr::filter(community == "Nullarbor microbial mantles") %>% 
  dplyr::mutate(community = "Microbial mantles") %>% 
  wastdr::wastd_POST("communitygazettal", api_url = prod)

# SCP26a  > Limestone ridges (SCP 26a)
tec_listing_cat %>% 
  dplyr::filter(community == "SCP26a") %>% 
  dplyr::mutate(community = "Limestone ridges (SCP 26a)") %>% 
  wastdr::wastd_POST("communitygazettal", api_url = prod)

# Riparian communities of springs and Pools Pilbara ??
# tec_cons_listing_categories %>% 
#     dplyr::filter(com_label == "Riparian communities of springs and Pools Pilbara")

not_a_tec <- "Nullarbor microbial mantles|SCP26a|Riparian communities of springs and Pools Pilbara"

tec_listing_cat %>% 
  dplyr::filter(!grepl(not_a_tec, community)) %>% 
  wastdr::wastd_POST("communitygazettal", api_url = prod)
```
