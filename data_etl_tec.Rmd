...
category_type <- tec$CATEGORY_TYPES %>% 
    dplyr::transmute(
        category_type_code = CT.TYPE  %>% as.character, 
        category_type = CT.DESC  %>% as.character, 
        authority = CT.AUTHORITY.CODE  %>% as.character
    )

conservation_status <- tec$CRITERIA_CODES %>%
    dplyr::transmute(
    conservation_status_code = CRIT.CODE  %>% as.character,
    conservation_status_explanation = CRIT.DESC  %>% as.character
    )

# If only there were like, a function to turn uppercase into lower- or title case...
authorities <- tec$AUTHORITIES # A lookup for upper cased authorities vs lower cased authorities. yeah.
endorsement <- tec$ENDORSED_BY # another lookup for lower case. 

# primary
gazettal_wa <- tec$CATEGORIES %>%
    dplyr::transmute(
        com_id = COM.NO  %>% as.integer(), 
        cat_effect_date = CAT.EFFECT.DATE  %>% as.character, 
        category_type_code = CAT.CT.TYPE  %>% as.character, 
    ...
    dplyr::left_join(com_short, by = "com_id") %>%
    dplyr::left_join(category_type, by = "category_type_code")
    
gazettal_iucn <- tec$CATEGORY_CRITERIA %>% 
    dplyr::transmute(
        com_id = COM.NO  %>% as.integer(), 
        effective_from = CAT.EFFECT.DATE  %>% as.character, 
        conservation_status_code = CC.CRIT.CODE  %>% as.character
    ) %>%
    dplyr::left_join(com_short, by = "com_id") %>%
    dplyr::left_join(conservation_status, by = "conservation_status_code")
...
com_title <- "Threatened Ecological Communities (TEC)"
...
cre_title <- "TEC Recommendations"
cre_fn <- "data/community_recommendations.csv"
com_recommendations %>% write_delim(cre_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = cre_title, upload = cre_fn)
cre_rid <- "fd96a053-523c-401d-8755-9116c55f5744"
ckanr::resource_update(cre_rid, cre_fn)

cpu_title <- "TEC Publications"
cpu_fn <- "data/community_publications.csv"
com_publications %>% write_delim(cpu_fn, delim = ",")
...
cr_title <- "TEC Reviews"
cr_fn <- "data/community_reviews.csv"
com_reviews %>% write_delim(cr_fn, delim = ",")
...
ch_title <- "TEC History"
ch_fn <- "data/community_history.csv"
com_history %>% write_delim(ch_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = ch_title, upload = ch_fn)
ch_rid <- "479d7358-d42d-4c4b-93ee-1bac39d21024"
ckanr::resource_update(ch_rid, ch_fn)

oa_title <- "TEC Occurrence Actions"
...
ore_title <- "TEC Occurrence Recommendations"
ore_fn <- "data/occurrence_recommendations.csv"
occ_recommendations %>% write_delim(ore_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = ore_title, upload = ore_fn)
ore_rid <- "747464e4-2434-459a-b654-52861b37095c"
ckanr::resource_update(ore_rid, ore_fn)

# Bugger that. Let's make this easier.
upload_to_ckan <- function(data, 
    ...
    if (is.null(resource_id)){
        ...
        if (is.null(dataset_id)){
            cat("Error: provide either resource ID to update, or dataset ID to create resource for.\n")
        } else {
...
upload_to_ckan(occ_extent, "TEC occurrence extent", dataset_id = d$id, resource_id = "502c74d7-32be-453f-aff6-c50aedd3deed")
upload_to_ckan(occ_additional_data, "TEC occurrence additional data", 
               dataset_id = d$id, resource_id = "8dd3454c-1bf0-4575-adcb-154b700c4283")
upload_to_ckan(occ_biol_process, "TEC biological process", dataset_id = d$id, resource_id = "5805a671-576a-4077-b595-35ca96d7ca96")
upload_to_ckan(occ_nonbiol_process, "TEC nonbiological process", dataset_id = d$id, resource_id = "a5341117-2735-4694-8cd4-e806f4c06668")
upload_to_ckan(fire_history, "TEC fire history", dataset_id = d$id, resource_id = "e062f57e-f6aa-4403-a682-875dab2f77f6")
upload_to_ckan(occ_fauna, "TEC occurence fauna", dataset_id = d$id, resource_id = "a0eddc66-5fef-4252-b49f-691422a88f28")
upload_to_ckan(occ_species, "TEC occurence species", dataset_id = d$id, resource_id = "e94f1e35-30da-4982-b851-ceb888eb41b3")
upload_to_ckan(com_threats, "TEC community threats", dataset_id = d$id, resource_id = "5426c45b-1dee-4547-a6e3-bec81585e52e")
upload_to_ckan(survey_threats, "TEC survey threats", dataset_id = d$id, resource_id = "5b795656-a240-402e-92b8-edbe54b3fee8")
upload_to_ckan(site_visit, "TEC site visit", dataset_id = d$id, resource_id = "85cef94b-d12e-4aff-bd24-446d1c0757db")
upload_to_ckan(voucher_specimens, "TEC voucher specimens", dataset_id = d$id, resource_id = "6b8fe493-8183-4fe8-a91a-67a569bf9c46")
upload_to_ckan(gazettal_wa, "TEC gazettal WA", dataset_id = d$id, resource_id = "f0085651-8c38-496e-a6bd-1adc7ae2f06a")
upload_to_ckan(gazettal_iucn, "TEC gazettal IUCN", dataset_id = d$id, resource_id = "9483f333-97fd-4975-a799-d44bf8bd252a")
```
