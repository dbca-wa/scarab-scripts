...
tec_bdy_cae <- tibble::tibble(
    ...
    description = ifelse(
        ...
        glue::glue("Boundary ID {tec_bdy$BDY_ID} named {tec_bdy$BDY_ID} for lack of sites within."),
        glue::glue("Boundary ID {tec_bdy$BDY_ID} named {tec_bdy$FIRST_S_ID} after first site observed within.")
    ),
...
```{r tec_cae_large}
overweight_communities <- tec_bdy_cae %>% 
...
fat_filter <- overweight_communities %>% 
  extract2("community") %>% 
  paste0(collapse = "|")
```
...
tec_buf_cae <- tibble::tibble(
    ...
    description = ifelse(
        ...
        glue::glue("Buffer ID {tec_buf$BDY_ID} named {tec_buf$BDY_ID} for lack of sites within."),
        glue::glue("Buffer ID {tec_buf$BDY_ID} named {tec_buf$FIRST_S_ID} after first site observed within.")
    ),
...
tec_sit_cae <- tibble::tibble(
    ...
    dplyr::filter(!grepl(site_name_typos, community))
```
