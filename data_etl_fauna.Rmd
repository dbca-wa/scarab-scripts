...
group <- fauna_data$`Group List`
# skimr::skim(group)

# TODO secondary signs
# TODO resolution > fauna$resolution - location accuracy
```
...
species <- fauna_data$`Species List` %>%
  dplyr::transmute(
    ...
    phylo_group_id = PhyloGroup %>% as.character(),
...
fauna_skim %>% filter(stat == "missing", value > 0) %>% select(-stat, -level, -formatted) %>% DT::datatable(.)
```

The table `fauna` contains species occurrence records:
* taxonomy,
* location (coordinates and derived information) and time of observation,
...
species_fn <- "data/fauna_species.csv"
species %>% 
    dplyr::select(-region_id_list, -starts_with("file_number")) %>% 
    write_delim(species_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id,
#                      format = "csv",
#                      name = "Fauna Species Names",
#                      upload = species_fn)
ckanr::resource_update("eae0b480-c687-450c-a736-87d4e75c2a4d", species_fn)

res_prio_fn <- "data/research_priorities.csv"
res_prio %>%
    dplyr::select(-region_id_list, -starts_with("file_number")) %>% 
    write_delim(res_prio_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id,
#                      format = "html",
#                      name = "Fauna Research Priorities",
#                      upload = res_prio_fn)
ckanr::resource_update("7a26d00c-f3da-46a3-9d74-42e4475bd6a9", res_prio_fn)
...
recplan_fn <- "data/recovery_plans.csv"
recovery_plans %>% write_delim(recplan_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id,
#                      format = "csv",
#                      name = "Recovery Plans",
#                      upload = recplan_fn)
ckanr::resource_update("9f8426c3-20ed-4ba7-a551-05e1838ab1cb", recplan_fn)

fauna_fn <- "data/fauna_occurrence.csv"
fauna %>% write_delim(fauna_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id,
#                      format = "csv",
#                      name = "Fauna Occurrence Data",
#                      upload = fauna_fn)
