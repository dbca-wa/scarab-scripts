...
conservation_categories %>% head(10) %>% DT::datatable(.)
```
...
fauna_cons_listing %>% dplyr::filter(name_id == 0)
```
...
save(fauna_cons_listing, file=fcl_file)
...
tsc_conscat_sp <- "conservationcategory" %>% 
  ...
  dplyr::filter(conservation_list %in% c(1,2,3,6,7))

# IUCN cons listing
# IUCN 2012 3
tfa_listing_cat_iucn <- fauna_cons_listing %>% 
...
tfa_listing_cat_iucn %>% wastdr::wastd_POST("taxon-conservationlisting", api_url = prod)
...
tfa_listing_cat_wawca %>% wastdr::wastd_POST("taxon-conservationlisting", api_url = prod)
...
tfa_listing_cat_spfn %>% wastdr::wastd_POST("taxon-conservationlisting", api_url = prod)
...
tfa_listing_cat_waps %>% wastdr::wastd_POST("taxon-conservationlisting", api_url = prod)
...
tfa_listing_cat_epbc %>% wastdr::wastd_POST("taxon-conservationlisting", api_url = prod)
