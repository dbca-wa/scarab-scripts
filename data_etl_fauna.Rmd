...
setdiff(fauna_cons_listing$taxon,tfa_listing_cat_iucn2001$taxon) # none missing from fauna_cons_listing
setdiff(tfa_listing_cat_iucn2001$taxon,fauna_cons_listing$taxon) # 275 taxon missing from tfa_listing_cat_iucn2001

str(fauna_cons_listing)

species_wo_crit<-tfa_listing_cat_iucn2001 %>% 
  dplyr::filter(criteria=="NULL") %>% 
  dplyr::transmute(
  name_id = taxon %>% as.character(),
  source_id = source_id %>% as.character,
  criteria = criteria
  ) 

species_miss_crit <- species_wo_crit %>%
  dplyr::left_join(fauna_cons_listing,by=("name_id"))#%>% 
  dplyr::group_by(list_code) %>% 
  dplyr::distinct(criteria_code) %>% 
  dplyr::arrange(list_code, criteria_code) %>% 
  dplyr::select(list_code, criteria_code)
readr::write_csv(species_miss_crit, path = here::here("data","tfa_conscrit_2001.csv"))

unique(species_wo_crit$name_id)# 57 unique species
