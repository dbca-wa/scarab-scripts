...
species <- fauna_data$`Species List` %>%
  dplyr::transmute(
    ...
    file_number_calm = CALMFileNum %>% as.character() %>% 
        map(~ as_filenumber(., prefix = "CALM")),
    file_number_dec = DECFileNum %>% as.character() %>% 
        map(~ as_filenumber(., prefix = "DEC")),
    file_number_dpaw = DPaWFileNum %>% as.character() %>% 
        map(~ as_filenumber(., prefix = "DPAW")),
    file_number_dbca = DBCAFileNum %>% as.character() %>% 
...
species %>% dplyr::select(
    # -region_id_list, 
    -starts_with("file_number")) %>% write_delim(sp_fn, delim = ",")
