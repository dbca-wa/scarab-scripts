...
conscat <- wastdr::wastd_GET("conservationcategory", api_url = prod) %>% 
  magrittr::extract2("response") %>% 
  httr::content() %>% 
  magrittr::extract2("results") %>% # will need to change to features
  {tibble::tibble(
    id = purrr::map_int(., "id"),
...
conscrit <- wastdr::wastd_GET("conservationcriterion", api_url = prod) %>% 
  ...
  {tibble::tibble(
    ...
    code = purrr::map_chr(., "code"),
    label = purrr::map_chr(., "label", .default = "todo")
  )} %>% 
  dplyr::arrange(list_id, rank) %>%
  dplyr::left_join(conslist, by = "list_id")
...
```
There are 0 criteria in the fauna cons listings which do not match a new tsc criteria. 
There is one criteria in the new tsc criteria which do not match a criteria in the fauna cons listings.
...
## This is the one we want to use to manual map all criteria
