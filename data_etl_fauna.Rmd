...
In particular, the following species names are missing a NameID.

TODO: Compare all species names (with and without NameID) against recent WACensus names and NameIDs.
Fix inconsistencies at source (Fauna db).
 
```{r species_missins_nameid}
species %>% dplyr::filter(is.na(name_id)) %>% kable
```
...
default_date <- parse_date_time("1900-01-01 00:00:00", orders = "ymd HMS", tz = tz)
...
### Data overview
The table `fauna` contains species occurrence records with information about:
...
* `r fauna %>% dplyr::filter(is.na(datetime)) %>% nrow()` datetimes couldn't be restored and require
  a valid date (and ideally time) to be set at the source.
* `r fauna %>% dplyr::filter(orig_time == "") %>% nrow()` records lack an explicit time and were 
  defaulted to midnight AWST.
* `r fauna %>% dplyr::filter(orig_date == "") %>% nrow()` records lack an explicit date and were
  defaulted to 1900-01-01.
* `r fauna %>% dplyr::filter(is.na(latitude)) %>% nrow` latitudes are missing.
* `r fauna %>% dplyr::filter(is.na(longitude)) %>% nrow` longitudes are missing.
* `r fauna %>% dplyr::filter(latitude > 0) %>% nrow` latitudes lack the minus sign and turn up in China.
...
upload_to_ckan(fauna, "Threatened Fauna Occurrence", d$id, 
