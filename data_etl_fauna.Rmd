...
```{r compare tsc conscrit listing to tfa conscrit}
# data.dir=paste("~/projects/scarab-scripts/data/")
# setwd(data.dir)
# dir()
# tfa_conscrit<-read_csv("tfa_conscrit.csv",)%>%
#   filter(list_code=="IUCN(2012)")
...
fauna_cons_listing %>% dplyr::filter(list_code == "WAPF") %>% DT::datatable() # = WAPS
fauna_cons_listing %>% dplyr::filter(list_code == "ActionPlan") %>% DT::datatable() # Discard those?
fauna_cons_listing %>% dplyr::filter(list_code == "") %>% DT::datatable() # Discard those?
fauna_cons_listing %>% dplyr::filter(list_code == "RedList") %>% DT::datatable() # Which IUCN version?
