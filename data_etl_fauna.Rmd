...
users <- tibble::tibble(
    username = c("AmyM", "AbbyT", "BrianaWingfield", 
                 "ChristineFreegard", "GeorginaA", "KellieMantle", 
                 "NickyMarlow", "PeterMawson", "PeterOrell"),
    initials = c("AM", "AT", "BW", 
                 "CF", "GA", "KM", 
                 "NM", "PM", "PO"),
    name = c("Amy Mutton", "Abby Thomas", "Briana Wingfield", 
...
# fauna_data$`Species List` %>% make_coldef()
species <- fauna_data$`Species List` %>%
  dplyr::transmute(
    ...
    file_number_calm = CALMFileNum %>% as.character() %>% map(~ as_filenumber(., prefix = "CALM")),
    file_number_dec = DECFileNum %>% as.character() %>% map(~ as_filenumber(., prefix = "DEC")),
    file_number_dpaw = DPaWFileNum %>% as.character() %>% map(~ as_filenumber(., prefix = "DPAW")),
    file_number_dbca = DBCAFileNum %>% as.character() %>% map(~ as_filenumber(., prefix = "DBCA")),
    category = Category %>% as.character(), # "J" "N" "E" "B" "F" "D" "I" "A" "G" "C" "H"
    origin = Origin %>% as.character(), # N I
    trans = Trans %>% as.character(), # E T
    region_id_list = Region %>% map(~ as.list(strsplit(., ",")[[1]] %>% as.integer())),
    taxon_id = TaxonId %>% as.logical(),
    incl = Incl %>% as.logical(),
    trap = Trap %>% as.logical(),
    sight = Sight %>% as.logical(),
    nbox = Nbox %>% as.logical(),
    sign = Sign %>% as.logical(),
    BANo = BANo %>% as.logical(),
    buffer = Buffer %>% as.character(), # NA 0 1 2 3
    est_life = EST.LIFE %>% as.integer() # NA  0 10  7 15  6  8
  ) %>%
...
* taxonomy (links to `name_id`, names, various hierarchy levels, 
  paraphyletic groups e.g. `phylo_group`)
...
* some lookups: 
  * `category` (`r unique(species$category)`), 
  * `origin` (`r unique(species$origin)`), 
  * `trans` (`r unique(species$trans)`), 
  * `buffer` (`r unique(species$buffer)`)
* corporate file numbers (what's in those files?)
* some logical checkboxes: incl, trap, sight, nbox, sign, taxon_id, BANo (T, F, NA)
...
* [Conservation] Recovery Plan > Species Names
...
etl_fn <- "data_etl_fauna.html"
etl_title <- "Fauna Data ETL"
# r <- ckanr::resource_create(package_id = d$id, format = "html", name = etl_title, upload = etl_fn)
ckanr::resource_update("761af921-94ad-4383-be2c-300713c37e98", etl_fn)

sp_fn <- "data/fauna_species.csv"
species %>% dplyr::select(-region_id_list, -starts_with("file_number")) %>% write_delim(sp_fn, delim = ",")
sp_title <- "Fauna Species Names"
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = sp_title, upload = sp_fn)
ckanr::resource_update("eae0b480-c687-450c-a736-87d4e75c2a4d", sp_fn)

rp_fn <- "data/research_priorities.csv"
res_prio %>% write_delim(rp_fn, delim = ",")
rp_title <- "Fauna Research Priorities"
# r <- ckanr::resource_create(package_id = d$id, format = "html", name = rp_title, upload = rp_fn)
ckanr::resource_update("7a26d00c-f3da-46a3-9d74-42e4475bd6a9", rp_fn)

cs_fn <- "data/conservation_status.csv"
cs_title <- "Conservation Status List"
conservation_status %>% write_delim(cs_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = cs_title, upload = cs_fn)
ckanr::resource_update("2793b1a8-eb55-41e7-8f8f-9cbc79a01243", cs_fn)

gaz_fn <- "data/gazettal.csv"
gaz_title <- "Conservation Status List"
gazettal %>% write_delim(gaz_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = gaz_title, upload = gaz_fn)
ckanr::resource_update("ac089664-de5a-4efa-af3c-ada0ec77ef1f", gaz_fn)

rp_fn <- "data/recovery_plans.csv"
rp_title <- "Recovery Plans"
recovery_plans %>% write_delim(rp_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = rp_title, upload = rp_fn)
ckanr::resource_update("9f8426c3-20ed-4ba7-a551-05e1838ab1cb", rp_fn)

fauna_fn <- "data/fauna_occurrence.csv"
fauna_title <- "Fauna Occurrence Data"
fauna %>% write_delim(fauna_fn, delim = ",")
# r <- ckanr::resource_create(package_id = d$id, format = "csv", name = fauna_title, upload = fauna_fn)
ckanr::resource_update("b73ae0de-f243-4bb3-bbce-25b65bb98e66", fauna_fn)
```
