...
malleefowl_data <- tfa_raw %>% dplyr::transmute(
  id = DBNo %>% as.integer(),
  malleefowl_MFeggsH = MFeggsH %>% as.character(),
  malleefowl_MFeggsU = MFeggsU %>% as.character(),
  malleefowl_MFmoundU = MFmoundU %>% as.character(),
...
There are `r nrow(tfa_occ_no_nameid)` distinct scientific fauna names without NameID, and `r nrow(tfa_occ_no_specode)` distinct scientific fauna names without a Species Code.
...
There are `r nrow(datum_tally)` different datums used across all distict fauna records. 
...
tfa_tally %>%
  ...
  dplyr::rename(tae = n) %>% 
  filter_(~tfa != tae) %>% reactable::reactable()
```
...
possum_diff <- tfa_possum %>% 
  anti_join(tae_possum,by = c("id"="source_id")) %>% reactable::reactable()
```
The two occurances above do not appear in tfa_tae becasue there is no lat or long data, which is a criteria for taxon area encounters
