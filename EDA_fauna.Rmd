...
if (file.exists(datafile)) {
  ...
  save(
    ...
    tfa_tcl, # TFA conservation listings with TSC criteria IDs
...
tfa_tcl_unique_taxa <- tsc_tcl_long$name_id %>%
  unique() %>%
  length() # 870
tsc_tcl_unique_taxa <- tsc_tcl_long$name_id %>%
  unique() %>%
  length()
```
...
TSC has `r nrow(tsc_tcl_long)` fauna conservation listings of 
