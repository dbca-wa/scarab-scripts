...
tfa_cons_listings <- tfa_data$Gazettal %>%
  dplyr::transmute(
    ...
    DateList == "",
    default_date,
    lubridate::parse_date_time2(DateList, orders, tz = tz, cutoff_2000 = 19L)) %>% 
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    delisted_on = ifelse(
    DateDeList == "",
    default_date,
    lubridate::parse_date_time2(DateDeList, orders, tz = tz, cutoff_2000 = 19L)) %>% 
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
    last_reviewed_on = ifelse(
    DateReview == "",
    default_date,
    lubridate::parse_date_time2(DateReview, orders, tz = tz, cutoff_2000 = 19L)) %>% 
      lubridate::as_datetime(.) %>% with_tz(tzone = tz),
...
trickytaxa <- tfa_listing_cat_iucn2012 %>% 
  dplyr::filter(taxon == 48126) 
...
trickytaxa %>% wastdr::wastd_POST("taxon-conservationlisting")
...
tfa_tae[1:10000,] %>% chunk_post(serializer = "occ-taxon-points", chunksize = 100)
...
tfa_phys_sample_res <- tfa_phys_sample %>% 
  wastd_occ_obs_post(obstype="PhysicalSample") 
  # , api_url = dev, api_token = Sys.getenv("WASTDR_API_DEV_TOKEN"))
# some TAE missing - MIlly to doublecheck all source_ids here exist in tfa_tae,
