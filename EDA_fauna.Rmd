...
tfa_cons_list_w_tsc <- tfa_cons_listings %>%
...
if (file.exists(datafile)) {
  ...
  save(
    ...
    tsc_taxa, # TSC taxonomic animal names (3655)
    tsc_tcl, # TSC taxon conservation listings (7841)


    tfa_data, # entire TFA database
    tfa_cons_list_w_tsc, # TFA conservation listings with TSC criteria IDs
    tfa_listing_cat_epbc, # TFA cons listings split up by cons list
    tfa_listing_cat_iucn1994,
    tfa_listing_cat_iucn2001,
    tfa_listing_cat_iucn2012,
    tfa_listing_cat_spfn,
...
tfa_tcl_unique_taxa <- tfa_tcl$name_id %>%
  unique() %>%
  length() # 870
tsc_tcl_unique_taxa <- tsc_tcl$name_id %>%
  unique() %>%
  length()
```
...
TFA has `r nrow(tfa_tcl)` conservation listings of `r tfa_tcl_unique_taxa`.
TSC has `r nrow(tsc_tcl)` conservation listings of `r tsc_tcl_unique_taxa`, 
including animals and plants *sensu latu*.
...
```{r}
tsc_tcl %>%
  dplyr::anti_join(tfa_tcl, by = "source_id") %>%
  reactable::reactable(filterable = TRUE)
```
...
```{r}
tfa_tcl %>% dplyr::filter(name_id == 0)
```

### TFA unmigrated conservatin listings
...
tsc_cl_sum_iucn1994 <- tsc_tcl %>% make_tsc_cl_summary("IUCN1994")
tsc_cl_sum_iucn2001 <- tsc_tcl %>% make_tsc_cl_summary("IUCN2001")
tsc_cl_sum_iucn2012 <- tsc_tcl %>% make_tsc_cl_summary("IUCN2012")
tsc_cl_sum_wawca <- tsc_tcl %>% make_tsc_cl_summary("WAWCA")
tsc_cl_sum_waps <- tsc_tcl %>% make_tsc_cl_summary("WAPS")
tsc_cl_sum_spfn <- tsc_tcl %>% make_tsc_cl_summary("SPFN")
tsc_cl_sum_epbc <- tsc_tcl %>% make_tsc_cl_summary("EPBC")
...
cons_cat_summary <- tfa_cons_cat_sum %>%
  ...
  cbind(Category = c("IUCN1994", "IUCN2001", "IUCN2012", "WAWCA", "WAPF", "SPFN", "EPBC")) %>%
  dplyr::select("Category", everything())

cons_cat_summary %>% reactable::reactable()
```

The data for each category_cache is tallied direct from the master TFA and TSC data sets. 
However, the cell labels are do not match due to different naming conventions in 
TFA and TSC, so this table must be manually checked by list-code e.g. 
IUCN1994 etc, for comparison.

```{r detailed_summary_conservation_categorys_and_listings}

# cons_cat_tally_tfa <- tfa_tcl %>%
#   dplyr::group_by(category_cache) %>%
#   dplyr::tally() %>%
#   dplyr::filter(category_cache != " ") %>%
#   dplyr::rename(tfa = category_cache)
# 
# cons_cat_tally_tsc <- tsc_tcl %>%
#   dplyr::group_by(category_cache) %>%
#   dplyr::tally() %>%
...
upload_file_to_ckan("e1c3119b-f44d-4244-8915-21097bd16b90", here::here("tfa_conscrit_w_tsc.csv"))
