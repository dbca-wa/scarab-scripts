...
species_notes_dup <- species_notes_with_duplicates %>%
  filter(species_code %in% unique(.[["species_code"]][duplicated(.[["species_code"]])])) # two duplicate species_codes- remove from species notes

readr::write_csv(species_notes_dup, path = here::here("data", "species_notes_duplications.csv"))
...
species <- tfa_data$`Species List` %>%
  ...
  dplyr::filter(!name_id == "34146") %>% # This is the removed name_id
...
# species_list_dup <- species %>%
#   filter(name_id %in% unique(.[["name_id"]][duplicated(.[["name_id"]])])) # one duplicate name_id
# 
# readr::write_csv(species_list_dup, path = here::here("data", "species_list_duplications.csv"))
