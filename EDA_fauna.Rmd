...
readr::write_csv(shires, 
                 path = here::here("data", "tfa_shires_data.csv")
  )
...
readr::write_csv(tenure, 
                 path = here::here("data", "tfa_tenure_data.csv")
  )
...
readr::write_csv(vegetation_types, 
                 path = here::here("data", "tfa_vegetation_types_data.csv")
  )
...
readr::write_csv(certainty, 
                 path = here::here("data", "tfa_certainty_data.csv")
  )
...
phylo_class <- tfa_data$`Class List` %>%
  ...
  dplyr::arrange(order)
readr::write_csv(phylo_class, 
                 path = here::here("data", "tfa_class_list_data.csv")
  )
...
# skimr::skim(group)
readr::write_csv(phylo_group, 
                 path = here::here("data", "tfa_group_list_data.csv")
  )
...
readr::write_csv(tfa_species_notes_duplications, path = here::here(
  "data", "tfa_species_notes_duplications_data.csv")
  )
...
readr::write_csv(category, path = here::here(
  "data", "tfa_category_list_data.csv")
  )
...
readr::write_csv(
  secondary_signs, 
  path = here::here("data", "tfa_secondary_signs_data.csv")
) 
...
readr::write_csv(tfa_species_list_dup, 
                 path = here::here("data", "tfa_species_list_duplications_data.csv"))
...
readr::write_csv(species, 
  path = here::here("data", "tfa_species_data.csv")
) 
...
readr::write_csv(species_names, 
  path = here::here("data", "tfa_species_names_data.csv")
) 
...
readr::write_csv(resolution, 
  path = here::here("data", "tfa_resolution_data.csv")
) 
...
readr::write_csv(tfa_cons_lists, 
  path = here::here("data", "tfa_cons_lists_data.csv")
) 
...
readr::write_csv(dec_tfa_cons_lists, 
  path = here::here("data", "tfa_dec_tfa_cons_lists_data.csv")
) 
...
readr::write_csv(tfa_cons_cat, 
  path = here::here("data", "tfa_cons_cat_data.csv")
)
...
readr::write_csv(tfa_cons_listings, 
  path = here::here("data", "tfa_cons_listings_data.csv")
)
...
tfa_occ <- tfa_data$`Fauna Records` %>% dplyr::transmute(
  ...
  invisible()
readr::write_csv(tfa_occ, 
                 path = here::here("data", "tfa_occ_data.csv"))
...
tfa_tae <- tfa_occ %>%
  ...
  # dplyr::mutate(status = case_when(status == "NEW" ~ "new",
  #                               status == "ACCEPTED" ~ "curated",
  #                               status == "READY" ~ "proofread",
...
readr::write_csv(tfa_act,
                 path = here::here("data", "tfa_recovery_actions_data.csv"))
readr::write_csv(tfa_pla, 
                 path = here::here("data", "tfa_recovery_plans_data.csv"))
```
...
readr::write_csv(tfa_resp,
                 path = here::here("data", "tfa_research_priorities_data.csv"))
readr::write_csv(tfa_rest, 
                 path = here::here("data", "tfa_research_topics_data.csv"))
```
...
tsc_conslist <- tsc_GET("conservationlist") %>%
   tsc_parse() %>% 
  dplyr::select(
...
tsc_landforms <- tsc_GET("lookup-landform") %>%
  tsc_parse() %>% 
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_confidence <- tsc_GET("lookup-confidence") %>%
  tsc_parse() %>% 
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_repro_mat <- tsc_GET("lookup-reproductivematurity") %>%
  tsc_parse() %>%
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_sec_signs <- tsc_GET("lookup-secondarysigns") %>%
  tsc_parse() %>% 
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_samp_dest <- tsc_GET("lookup-sampledestination") %>%
  tsc_parse() %>%
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_samp_type <- tsc_GET("lookup-sampletype") %>%
  tsc_parse() %>%
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tsc_det_meth <- tsc_GET("lookup-detectionmethod") %>%
  tsc_parse() %>%
  dplyr::rename(
    list_id = id,
    list_code = code,
...
tfa_anim_obs_complete <- anim_obs %>%
  ...
  mutate(no_juvenile_unknown = replace(no_juvenile_unknown, which(no_juvenile_unknown<0), 10)) %>% # make negative value a positive
...
tfa_veg_class_complete <- veg_class %>% 
  ...
  dplyr::mutate(obstype = "VegetationClassification") %>% 
  invisible()
...
#   tsc_POST("taxon-conservationlisting")
...
tfa_listing_cat_iucn2012_res <- tfa_listing_cat_iucn2012 %>% 
  tsc_chunk_post("taxon-conservationlisting")
...
```{r load_eoo, eval=F}
tfa_eoo %>% dplyr::select(-data, -eoo_sfc) %>% tsc_POST("taxon")
```
...
tsc_POST(taxonpoint, serializer = "occ-taxon-points")
...
# LONG RUNNING UPLOAD
tfa_tae_res <- tfa_tae[70000:107965,] %>% 
  tsc_chunk_post(serializer = "occ-taxon-points", chunksize = 1000)

# tfa_tae[100001:nrow(tfa_tae),] %>% tsc_chunk_post(serializer = "occ-taxon-points", chunksize = 100)
...
#   tsc_occ_obs_post(obstype="PhysicalSample")
# tfa_phys_sample_res <- tfa_phys_sample[1:100,] %>%
#   tsc_occ_obs_post(obstype="PhysicalSample")

# Bulk upload
# LONG RUNNING UPLOAD - RE-RUNS CREATE DUPLICATES
tfa_phys_sample_res <- tfa_phys_sample_complete %>% 
  tsc_occ_obs_post(obstype="PhysicalSample", chunksize = 1000) 

tfa_anim_obs_res <- tfa_anim_obs_complete %>% 
  tsc_occ_obs_post(obstype="AnimalObservation", chunksize = 1000)

# We cannot map hand-written AssSpecies to TSC NameIDs.
# tfa_ass_species_res <- tfa_ass_species[1,] %>% 
#   tsc_occ_obs_post(obstype="AssociatedSpecies")

tfa_veg_class_res <- tfa_veg_class_complete %>% 
  tsc_occ_obs_post(obstype="VegetationClassification", chunksize = 1000)

tfa_hab_comp_res <- tfa_hab_comp_complete %>% 
  tsc_occ_obs_post(obstype="HabitatComposition", chunksize = 1000)

# Export as (confidential) test data for API
tfa_phys_sample %>% readr::write_csv(path = here::here("data/tfa_phys_sample.csv"))
tfa_anim_obs %>% 
  dplyr::select(-"secondary_signs") %>% 
  readr::write_csv(path = here::here("data/tfa_anim_obs.csv"))
...
# Taxonomic names and taxon conservation listings for fauna from TSC
  ...
  tsc_tcl_fauna <-
    tsc_GET("taxon-conservationlisting") %>%
    tsc_parse()  %>%
     dplyr::mutate(taxon = taxon %>% as.character()) %>%
    dplyr::inner_join(tsc_taxa_fauna, by = "taxon")
  
tsc_tae_data <- tsc_GET("occ-taxon-points") # 137k records
  ...
  tsc_occ_fauna <- tsc_tae %>% 
   ...
   # Fauna occurrence observations
  tsc_tae_observations <- "occ-observation" %>% 
    tsc_GET() %>%
    tsc_parse()
  
  tsc_tae_observations_fauna <- tsc_tae_observations %>% 
    tidyr::unnest_wider(encounter,names_repair = "universal")
  
  tsc_tae_observations_fauna_2 <- tsc_tae_observations_fauna %>% 
    tidyr::unnest_wider(properties,names_repair = "universal")
  
  tsc_fauna_observations <- tsc_tae_observations_fauna_2 %>% 
    filter(source == 10)
...
length(unique(tsc_occ_fauna$source_id)) # 107965
...
#### Identify missing tae in tsc_tae
...
# tfa_tae_create_res <- tfa_tae_create %>% tsc_chunk_post(serializer = "occ-taxon-points", chunksize = 100)
```
...
WAWCA
This was not migrated becasue it is a duplicate of the other conservation listing information
...
WAWCA
This was not migrated because it is a duplicate of the other conservation listing information
...
tfa_tae_default_datetime <- tfa_tae %>% 
  filter(encountered_on == "1900-01-01 00:00:00+08:00") %>% as_tibble()
tfa_tae_default_datetime %>% rt
```
...
tsc_phys_sample_sum <-
  tsc_fauna_observations %>% make_tsc_occ_obs_summary("PhysicalSample")
tsc_anim_obs_sum <-
  tsc_fauna_observations %>% make_tsc_occ_obs_summary("AnimalObservation")
tsc_veg_class_sum <-
  tsc_fauna_observations %>% make_tsc_occ_obs_summary("VegetationClassification")
tsc_hab_comp_sum <-
  tsc_fauna_observations %>% make_tsc_occ_obs_summary("HabitatComposition")
...
occ_obs_summary <- tfa_occ_obs_sum %>%
  ...
  cbind(Obstype = c("PhysicalSample", "AnimalObservation", "VegetationClassification","HabitatComposition")) %>% # PlantCount
...
tfa_species_phys_sample <- tfa_phys_sample_complete %>% 
  filter(source_id %in% c("117", "10403", "70138")) %>% 
  transmute(
    source_id = source_id,
    tfa_sample_destination= sample_destination,
    tfa_sample_label = sample_label)

tfa_species_anim_obs <- tfa_anim_obs %>% 
  filter(source_id %in% c("117", "23126", "100008")) %>%
  transmute(
    source_id = source_id,
    tfa_detection_method = detection_method,
    tfa_no_adult_male = no_adult_male)

tfa_species_veg_class <- tfa_veg_class_complete %>% 
  filter(source_id %in% c("67", "96871", "73085")) %>%
  transmute(
    source_id = source_id,
    tfl_level1= level1)

tfa_species_hab_comp <- tfa_hab_comp_complete %>% 
  filter(source_id %in% c("46", "74702", "85099")) %>%
  transmute(
    source_id = source_id,
    tfl_landform = landform)

tsc_species_phys_sample <- tsc_fauna_observations %>% 
  filter(obstype == "PhysicalSample") %>% 
  filter(source_id %in% c("117", "10403", "70138")) %>% 
  transmute(
    source_id = source_id,
    tsc_sample_destination = sample_destination,
    tsc_sample_label = sample_label)

tsc_species_anim_obs <- tsc_fauna_observations %>% 
  filter(obstype == "AnimalObservation") %>% 
  filter(source_id %in% c("117", "23126", "100008")) %>% 
  transmute(
    source_id = source_id,
    tsc_criteria = detection_method %>% as.character(),
    tsc_no_adult_male = no_adult_male) %>%
  dplyr::left_join(tfa_det_meth, by = "tsc_criteria") %>%
  select(c("source_id",
           "tsc_detection_method",
           "tsc_no_adult_male"))

tsc_species_veg_class <- tsc_fauna_observations %>% 
  filter(obstype == "VegetationClassification") %>% 
  filter(source_id %in% c("67", "96871", "73085")) %>%
  transmute(
    source_id = source_id,
    tfl_level1= level1)

tsc_species_hab_comp <- tsc_fauna_observations %>% 
  filter(obstype == "HabitatComposition") %>% 
  filter(source_id %in% c("46", "74702", "85099")) %>%
  transmute(
    source_id = source_id,
    tsc_criteria = landform %>% as.character()) %>% 
  dplyr::left_join(tfa_landcrit, by = "tsc_criteria") %>% 
  select(c("source_id","tsc_landform_id")) 
```

Physical sample

```{r}
tfa_phys_sample_sum <- tfa_species_phys_sample %>% 
  left_join(tsc_species_phys_sample, by = "source_id")

tfa_phys_sample_sum %>% rt
```

Animal observations

```{r}
tfa_anim_obs_sum <- tfa_species_anim_obs %>% 
  left_join(tsc_species_anim_obs, by = "source_id")
...
```{r}
tfa_hab_veg_class <- tfa_species_veg_class %>% 
  left_join(tsc_species_veg_class, by = "source_id")

tfa_hab_veg_class %>% rt
```

Habitat composition

```{r}
tfa_hab_comp_sum <- tfa_species_hab_comp %>% 
...
upload_to_ckan(category, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "ef713623-912f-409c-92fd-13f0e7d5c88b"
)
upload_to_ckan(certainty, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "0cef8910-daa7-42a6-990f-be9ba337600b"
)
upload_to_ckan(phylo_class, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "80914d52-cfaa-4b13-a1e9-1dda84ce73f3"
)
upload_to_ckan(phylo_group, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "5f69a428-dc17-4617-9443-0d932b528a90"
)
upload_to_ckan(species, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "db2a494b-9143-40fa-a81b-a17cd1ea04cb"
)
upload_to_ckan(species_names, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "9a30cf91-cb0b-4321-aeb0-90eab170b023"
)
upload_to_ckan(dec_tfa_cons_lists, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "6a3b2d20-692f-4e0d-8cdb-08a11567a836"
)
upload_to_ckan(tfa_cons_lists, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "10482953-443e-4917-b069-05dad1399d19"
)
upload_to_ckan(tfa_cons_listings, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "ac089664-de5a-4efa-af3c-ada0ec77ef1f"
)
upload_to_ckan(tfa_cons_cat, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "81709dfb-ab13-4ea3-9716-c1e75db17211"
)
upload_to_ckan(tenure, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "93de5d9e-68d4-41e3-b7b5-20b1c7f6c8d5"
)
upload_to_ckan(tfa_act, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "80b0734b-9d03-4592-8519-9877923f495c"
)
upload_to_ckan(tfa_pla, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "705553d8-92eb-4e01-ba0e-4b39e6b44918"
)
upload_to_ckan(tfa_resp, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "02798015-7395-464d-8db1-cfe53196a69f"
)
upload_to_ckan(tfa_rest, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "66936696-a1e9-4c59-b67f-623b21f1b0c7"
)
upload_to_ckan(resolution, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "f8819c5a-ae56-4d15-9be3-e1c52ac11fb0"
)
upload_to_ckan(secondary_signs, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "562251ae-e09a-4d5e-9b64-272c7316e9a2"
)
upload_to_ckan(shires, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "3a588c2d-f143-486e-be47-92bfa46f6433"
)
upload_to_ckan(vegetation_types, "threatened-and-priority-fauna-database",
  d$id,
  resource_id = "78cce603-6d8a-409c-8c9d-dfecb737b9f9"
)
upload_to_ckan(tfa_occ, "threatened-and-priority-fauna-database",
  d$id,
