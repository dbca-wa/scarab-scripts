...
Some conservation listings were not parsed. ActionPlan and RedList are not part of TSC. ActionPlan is currently a paper resource and RedList a separate online resource that cannot, as yet, be parsed to TSC. BCA?
...
unique(tfa_cons_list_w_tsc$list_code)
```

**QA** Conservation criteria were manually mapped for each species, therefore they need to be checked to confirm whether the mapping is correct. In addition, some conservation criteria could not be mapped as they do not exist in the list/category_code. These need to be corrected before the manual mapping can be updated and the correct criteria assigned for all species.

```{r }
query_manual_mapping <- tfa_data %>% 
  dplyr::select(c(source_id,name_id,list_code,category_code,criteria_code,assigned_as_in_tsc,comments_MP,actions))

query_manual_mapping %>% 
  reactable::reactable(filterable = TRUE) 
```
...
TFA <- as.data.frame(c(tfa_effective_from,tfa_effective_to,tfa_last_reviewed_on))
TSC <- as.data.frame(c(tsc_effective_from,tsc_effective_to,tsc_last_reviewed_on))

dates <- TFA %>% 
  rbind(TSC) %>% 
  cbind(Database = c("TFA","TSC")) %>% 
  select("Database", everything())
 
dates %>% 
  reactable::reactable(columns = list(effective_from = colDef(format = colFormat(date = TRUE)),
                                      effective_to = colDef(format = colFormat(date = TRUE)),
                                      last_reviewed_on = colDef(format = colFormat(date = TRUE))))
```

Summary of the number of conservation listings for each category found in TFA and TSC. 

Number of records for each conservation list for TFA and TSC is tallied and combined in a table.

```{r summary_conservation_list}
iucn1994 <-as.data.frame(tfa_listing_cat_iucn1994 %>% 
  nrow())
colnames(iucn1994) <- "TFA"
iucn2001 <-as.data.frame(tfa_listing_cat_iucn2001%>% 
  nrow())
colnames(iucn2001) <- "TFA"
iucn2012 <-as.data.frame(tfa_listing_cat_iucn2012 %>% 
  nrow())
colnames(iucn2012) <- "TFA"
wawca <-as.data.frame(tfa_listing_cat_wawca %>% 
  nrow())
colnames(wawca) <- "TFA"
waps <-as.data.frame(tfa_listing_cat_waps %>% 
  nrow())
colnames(waps) <- "TFA"
spfn <-as.data.frame(tfa_listing_cat_spfn %>% 
  nrow())
colnames(spfn) <- "TFA"
epbc <-as.data.frame(tfa_listing_cat_epbc %>% 
  nrow())
colnames(epbc) <- "TFA"

iucn1994_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "IUCN1994")) %>%
  nrow())
colnames(iucn1994_TSC) <- "TSC"

iucn2001_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "IUCN2001")) %>%
  nrow())
colnames(iucn2001_TSC) <- "TSC"

iucn2012_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "IUCN2012")) %>%
  nrow())
colnames(iucn2012_TSC) <- "TSC"

wawca_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "WAWCA")) %>%
  nrow())
colnames(wawca_TSC) <- "TSC"

waps_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "WAPS")) %>%
  nrow())
colnames(waps_TSC) <- "TSC"

spfn_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "SPFN")) %>%
  nrow())
colnames(spfn_TSC) <- "TSC"

epbc_TSC <- as.data.frame(tsc_data %>% 
  dplyr::select(category_cache) %>% 
  dplyr::filter(str_detect(category_cache, "EPBC")) %>%
  nrow())
colnames(epbc_TSC) <- "TSC"

cons_cat_TFA <- rbind(iucn1994,iucn2001,iucn2012,epbc,spfn,wawca,waps)
cons_cat_TSC <- rbind(iucn1994_TSC,iucn2001_TSC,iucn2012_TSC,epbc_TSC,spfn_TSC,wawca_TSC,waps_TSC) 

cons_cat_summary <- cons_cat_TFA %>% 
  cbind(cons_cat_TSC) %>% 
  cbind(Category = c("IUCN1994","IUCN2001","IUCN2012","EPBC","SPFN","WAWCA","WAPF")) %>% 
  dplyr::select("Category", everything())

cons_cat_summary %>% reactable::reactable()
```

The data for each category_cache is tallied direct from the master TFA and TSC data sets. However, the cell labels are do not match due to different naming conventions in TFA and TSC, so this table must be manually checked by list-code e.g. IUCN1994 etc, for comparison.

```{r detailed_summary_conservation_categorys_and_listings}

cons_cat_tally_tfa <- tfa_data %>% 
  dplyr::group_by(category_cache) %>% 
  dplyr::tally() %>% 
  dplyr::filter(category_cache != " ") %>% 
  dplyr::rename(tfa = category_cache)

cons_cat_tally_tsc <- tsc_data %>% 
  dplyr::group_by(category_cache) %>% 
  dplyr::tally()%>% 
  dplyr::rename(tsc = category_cache)

cons_cat_tally <- cons_cat_tally_tfa %>% 
  dplyr::bind_cols(cons_cat_tally_tsc)

cons_cat_tally %>% reactable::reactable(filterable = F)
```
...
[Threatened Fauna Dataset](https://data.dpaw.wa.gov.au/dataset/threatened-and-priority-fauna-database/) on the DBCA data catalogue.


```{r upload_data_catalogue}
fn <- "EDA_fauna.html"
rid <- "2b136da7-35cf-44a5-b29e-9f568176b7ad"

if (fs::file_exists(fn)) {
  message("Uploading this workbook to data catalogue...")
  r <- ckanr::resource_update(rid, fn)
  glue::glue("Updated {r$name} at\n{r$url}") %>% as.character() %>% message
}
```
