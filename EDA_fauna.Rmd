...
if (file.exists(tfa_file)) {
  ...
  save(tfa_data, snapshot_last_updated, file=tfa_file)
}
...
tfa_occ <- tfa_data$`Fauna Records` %>% dplyr::transmute(
  ...
  datetime = Date %>% parse_as_datetime() %>% 
...
## TODO FM: compare tfa_occ to tfa_tae - duplicates? missing?
```
...
tfa_occ_map <- tfa_occ %>% has_name_id_and_location
...
```{r savepoint_tfa}
tfa_datafile <- here::here("data/tfa_all.RData")
save(
  all_crit_map,
anim_fauna,
anim_obs,
...
tfa_rest,
tfa_tae,
tsc_cl_fauna_datafile,
tsc_occ_fauna_datafile,
tz,
uat,
upload_file_to_ckan,
upload_to_ckan,
...
tfa_tae_res <- tfa_tae %>% chunk_post(serializer = "occ-taxon-points", chunksize = 100)
```
...
tfa_phys_sample_res <- tfa_phys_sample %>% 
  wastd_occ_obs_post(obstype="PhysicalSample") 
  # , api_url = dev, api_token = Sys.getenv("WASTDR_API_DEV_TOKEN"))
# Florian to re-upload tfa_tae to TSC
# Florian to create: get TSC TAE source and sourceID, then compare locally
# to determine what to create, what to update (if ever), what to skip
...
if (file.exists(tsc_occ_fauna_datafile)) {
  ...
  tsc_tae <- tsc_tae_data$data %>%
    {
    tibble::tibble(
      source = purrr::map_chr(., c("properties", "source"), .default = NA),
      source_id = purrr::map_chr(., c("properties", "source_id"), .default = NA),
      taxon = purrr::map_chr(., c("properties", "taxon"), .default = NA),
      code = purrr::map_chr(., c("properties", "code"), .default = NA),
      name = purrr::map_chr(., c("properties", "name"), .default = NA),
      label = purrr::map_chr(., c("properties", "label"), .default = NA),
      description = purrr::map_chr(., c("properties", "description"), .default = NA),
      status = purrr::map_chr(., c("properties", "status"), .default = NA),
      encounter_type = purrr::map_chr(., c("properties", "encounter_type"), .default = NA),
      encountered_on = purrr::map_chr(., c("properties", "encountered_on"), .default = NA),
      encountered_by = purrr::map_chr(., c("properties", "encountered_by"), .default = NA),
      area_type = purrr::map_chr(., c("properties", "area_type"), .default = NA),
      geolocation_capture_method = purrr::map_chr(., c("properties", "geolocation_capture_method"), .default = NA)
      # longitude = purrr::map_chr(., c("geometry", "coordinates", 1L), .default = NA),
      # latitude = purrr::map_chr(., c("geometry", "coordinates", 2L), .default = NA)
    )
    }
  ...
  # nrow(tsc_tae) 150987 - all fauna + flora taxa occurrences in TSC
  # nrow(tsc_occ_fauna) 106108 - all fauna taxa occurrences joined to tsc_taxa_fauna
  # this is puzzling
  # length(unique(tsc_occ_fauna$source_id)) #102563
...
tsc_occ_fauna_duplicate_source_ids <- tsc_occ_fauna %>%
...
tfa_tae_create %>% chunk_post(serializer = "occ-taxon-points")
```
...
tfa_occ_no_time<- tfa_occ %>% 
    filter(orig_time == "1900-01-01 00:00:00")
...
# tfa_occ_bad_location <- tfa_occ %>% 
#   impossible_location() #4
# 
# tfa_occ_missing_name_id <- tfa_occ %>% 
#   filter(is.na(name_id)) #1
# 
# tfa_occ_missing_lat_or_long <- tfa_occ %>% 
#   filter(is.na(latitude) |
#             is.na(longitude)) #3

# The above eight records are the eight records we remove from tfa_occ prior 
# to creating tfa_tae and are therefore not migrated. Amy to correct asap- email her.
```
...
compared to `tfa_occ`. These missing occurrences come from 
...
missing <- tfa_tae %>% 
  dplyr::filter(taxon == 48070)
to_many <- tsc_occ_fauna %>% 
  dplyr::filter(taxon == 48070)
extra <- missing %>% 
  dplyr::anti_join(to_many, by = "source_id")
