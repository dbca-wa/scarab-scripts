...
```{r load_tfa_occ, message=FALSE, warning=FALSE}
...
ass_species <- tfa_occ %>%
  ...
  dplyr::filter(associated_species_1 != "" | associated_species_2 != "" | associated_species_3 != "" | associated_species_4 != "" | associated_species_5 != "")
...
phys_sample <- tfa_occ %>% 
  ...
  dplyr::filter(specimen_voucher_exists != "FALSE" | sample_destination != "") %>% 
  dplyr::filter(specimen_voucher_exists != "FALSE" | sample_destination != "Not retained") %>% 
  dplyr::filter(sample_type != "No specimen" | sample_destination != "Not retained")%>% 
  dplyr::filter(sample_type != "Unknown" | sample_destination != "Unknown")%>% 
  dplyr::filter(sample_type != "Unknown" | sample_destination != "Unknown location")
...
# Extract and transform criteria Data from TSC
...
tfa_anim_obs <- anim_obs %>%
  ...
  dplyr::select(
    source,
    source_id,
    tsc_species_id_confidence,
    observation_details,
    tsc_detection_method,
    tsc_reproductive_maturity,
    tsc_secondary_signs,
    distinctive_features,
    number_of_adult_males,
    number_of_adult_females,
    number_of_adult_unknown,
    number_of_juvenile_males,
    number_of_juvenile_females,
  ...
  dplyr::rename(secondary_signs = tsc_secondary_signs) %>% 
...
```{r tfa_cons_listing_subsetting, message=FALSE}
...
```{r prepare eoo, eval=F, warning=FALSE, message=FALSE}
# extract all eoo
fna <- tfa_occ %>% 
...
```{r, eval = FALSE}
tfa_tae[1:100,] %>% chunk_post(serializer = "occ-taxon-points")

tfa_tae[1:1000,] %>% chunk_post(serializer = "occ-taxon-points")
...
```{r, eval=FALSE}
# I can't get this section to run successfully
tfa_phys_sample %>% wastdr::wastd_bulk_post("occ-observation")

tfa_anim_obs %>% wastd_bulk_post("occ-observation")

tfa_ass_species %>% wastd_bulk_post("occ-observation")

tfa_veg_class %>% wastd_bulk_post("occ-observation")

tfa_hab_comp %>% wastd_bulk_post("occ-observation")
```
...
```{r extract_tsc_conslist, message=FALSE}
...
datafile_tsc_fauna <- here::here("data/fauna_tsc.RData")
...
if (file.exists(datafile_tsc_fauna)) {
  ...
  save(tsc_taxa_fauna, tsc_tcl_fauna, file = datafile_tsc_fauna, compress="xz")
}
```
...
datafile.fauna <- here::here("data/fauna_tae.RData")

if (file.exists(datafile.fauna)) {
  load(datafile.fauna)
} else {
...
tsc_tae <- tsc_tae_data %>% 
  ...
  dplyr::mutate(taxon = taxon %>% as.character()) #%>%  sf_as_tbl()
...
save(tsc_tae, tsc_taxa_fauna, tsc_areas, file=datafile.fauna, compress="xz")
}
```

```{r munge, message = FALSE, warning=FALSE}
datafile.fauna <- here::here("data/fauna_tae.RData")
if (fs::file_exists(datafile.fauna)) load(datafile.fauna)
...
tsc_occ_fauna <- tsc_tae%>% 
  ...
  dplyr::inner_join(tsc_taxa_fauna, by = c("taxon")) %>% na.omit("source_id") # %>% sf_as_tbl()

## prior to right_join there are 93977 obs
...
tfa_landcrit %>%
  reactable::reactable(filterable = TRUE)
```
Landform terms were mapped to 'umbrella' terms which generally follow the morphological types listed in McDonald et al. (1998, p13). However, some of the terms could not be mapped as they are vegetation types (e.g. FOR = Forest) and not landform types. 
...
Sample destination was manually mapped to a generic field for its location and the specific location added to the sample label field as free text. We have done this because, combined with Flora, there are > 100 unique locations for sample destination, which would make a very long drop down menu.
...
Any records in the following table indicate occurrences in TFA that
...
missing_occ_tfa_tae %>% reactable::reactable(filterable = TRUE)

```

There are `r nrow(missing_occ_tfa_tae)` occurrences missing from tfa_tae when compared to tfa_occ. The occurrences listed were likely not migrated because they do not contain lat or long data which is a requirement for valid taxon area encounters.
...
tfa_occ_tally %>%
  dplyr::rename(tfa = n) %>% 
  left_join(tfa_tae_tally,by = "name_id") %>% 
  dplyr::rename(tae = n) %>% 
  filter(tfa != tae) %>% reactable::reactable()
```

### TFA_TAE occurrences not in TSC

Any records in the following table indicate taxon in TFA that were not migrated into TSC.

#### Name mismatches tfa_tae to tsc_tae
```{r name mismatches tfa_tae to tsc_tae}
missing_taxa_tsc <- tfa_tae %>% 
    dplyr::anti_join(tsc_tae, by = "taxon") 

missing_taxa_tsc_unique <- missing_taxa_tsc %>% 
    dplyr::select(taxon) %>% 
    distinct() %>% 
  dplyr::left_join(tsc_taxa_fauna,by = "taxon")#26

missing_taxa_tsc_unique %>% reactable::reactable()

```

There are `r nrow(missing_taxa_tsc)` taxon records missing from tsc_tae when compared to tfa_tae. These records are attributed to `r nrow(missing_taxa_tsc_unique)` unique species.  

#### Name mismatches tfa_tae to tsc_occ_fauna
```{r name mismatches tfa_tae to tsc_occ_fauna}
missing_taxa_occ_fauna <- tfa_tae %>% 
    dplyr::anti_join(tsc_occ_fauna, by = "taxon") 

missing_taxa_occ_fauna_unique <- missing_taxa_occ_fauna %>% 
    dplyr::select(taxon) %>% 
    distinct() %>% 
  dplyr::left_join(tsc_taxa_fauna,by = "taxon")#152

missing_taxa_occ_fauna_unique %>% reactable::reactable()

```

There are `r nrow(missing_taxa_occ_fauna)` taxon records missing from tsc_occ_fauna when compared to taf_tae. These records are attributed to `r nrow(missing_taxa_occ_fauna_unique)` unique species.  

#### Occurrence mismatches tfa_tae to tsc_tae
```{r missing_occurrences tfa_tae to tsc_tae}

missing_occ_tae_tsc <- tfa_tae %>%
...
missing_occ_tae_tsc_unique <- missing_occ_tae_tsc %>% 
    dplyr::select(taxon) %>% 
    distinct() %>% 
  dplyr::left_join(tsc_taxa_fauna,by = "taxon") #27

```

There are `r nrow(missing_occ_tae_tsc)` unique occurrences missing from tsc_tae when compared to tfa_tae. These missing occurrences come from `r nrow(missing_occ_tae_tsc_unique)` unique species.
...
tfa_tsc_tae_tally <- tfa_tae_tally %>%
  dplyr::rename(tfa = n) %>% 
  left_join(tsc_tae_tally,by = "source_id") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tfa != tsc) ## Can't make this into a reactable without a way of changing as sfc feature to a tbl- Florian?
```

#### Occurrence mismatches tfa_tae to tsc_occ_fauna
```{r missing_occurrences tfa_tae to tsc_occ_fauna}

missing_occ_tfa_occ_fauna <- tfa_tae %>%
  dplyr::anti_join(tsc_occ_fauna, by = "source_id") #39073

missing_occ_tfa_occ_fauna %>% reactable::reactable(filterable = T)
# missing_occ_tae_tsc <- tsc_tae %>%
#   dplyr::anti_join(tsc_occ_fauna, by = "source_id") 

missing_occ_tfa_occ_fauna_unique <- missing_occ_tfa_occ_fauna %>% 
    dplyr::select(taxon) %>% 
    distinct() %>%  
   dplyr::left_join(tsc_taxa_fauna,by = "taxon") #381

```

There are `r nrow(missing_occ_tfa_occ_fauna)` unique occurrences missing from tsc_occ_fauna when compared to tfa_tae. These missing occurrences come from `r nrow(missing_occ_tfa_occ_fauna_unique)` unique species.

The table below is a tally of mismatching occurrences and confirms the missing occurrences identified above.

```{r tally_tfa_tae_occ_fauna}
tfa_tae_tally <- tfa_tae %>%
  dplyr::group_by(source_id) %>% 
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>% 
...
tsc_occ_fauna_tally <- tsc_occ_fauna %>%
  dplyr::group_by(source_id) %>% 
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>% 
  ungroup() 

tfa_tae_tsc_occ_tally <- tfa_tae_tally %>%
  dplyr::rename(tfa = n) %>% 
  left_join(tsc_occ_fauna_tally,by = "source_id") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tfa != tsc) ## Can't make this into a reactable without a way of changing as sfc feature to a tbl- Florian?
