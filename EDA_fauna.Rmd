...
names_tfa_occ <- (as.data.frame(names(tfa_occ)))
...
# ## extract_landforms, message=FALSE}
# tsc_landforms <- "landforms" %>%
...
fire_hist <- tfa_occ %>%
  ...
  dplyr::transmute(
    source = 10,
    source_id = id,
    fire_history = fire_history) #There is a problem with migrating this- won't match TSC card

fire_hist %>%
  dplyr::select(fire_history) %>%
  distinct() %>% 
  readr::write_csv(path = here::here("data", "tfa_firecrit.csv"))
...
veg_class <- tfa_occ %>%
  ...
  dplyr::transmute(
    ...
    vegetation_type = vegetation_type) #There is a problem with migrating this- won't match TSC card
...
anim_obs <- tfa_occ %>%
  ...
  dplyr::transmute(
    source = 10,
    source_id = id,
    species_id_confidence = glue::glue(
        "Certainty_ID: {certainty_id};\n",
        "Certainty: {certainty}"), ## this needs to be resolved as neither fit the card in TSC
    observation_details = glue::glue(
        "Number seen: {number_seen};\n",
        "Comments: {observation_comments}"), # Can I do this, looks like free text on the card?
    ...
    detection_method = glue::glue(
      "Method: {observation_method};\n",
      "Type: {observation_type}"), # needs to be resolved and considered with secondary_signs
...
# ## extract_anim_obs, message=FALSE}
# tsc_anim_obs <- "observations" %>%
#   wastdr::wastd_GET(api_url = prod) %>%
#   wastdr::wastd_parse()
...
phys_sample <- tfa_occ %>% 
  ...
  dplyr::transmute(
    ...
    sample_label = specimen_voucher_catalogue_number)
