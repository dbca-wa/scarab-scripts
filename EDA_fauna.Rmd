...
glimpse(tsc_taxa_fauna)

unique(tfa_tae$taxon) #892
unique(tsc_taxa_fauna$taxon) #3655
intersect(tsc_taxa_fauna$taxon,tfa_tae$taxon) # 892 similar
setdiff(tsc_taxa_fauna$taxon,tfa_tae$taxon) # 2758 taxon in tsc that are not in tfa
setdiff(tfa_tae$taxon,tsc_taxa_fauna$taxon) # 0 taxon in tfa that are not in tsc
...
unique(tsc_occ_fauna$taxon) #738
intersect(tsc_occ_fauna$taxon,tfa_tae$taxon) # 738 similar
setdiff(tsc_occ_fauna$taxon,tfa_tae$taxon) # 0 taxon in tsc that are not in tfa
setdiff(tfa_tae$taxon,tsc_occ_fauna$taxon) # 144 taxon in tfa that are not in tsc
...
missing_taxon_tsc_occ <- missing_occ_tsc %>% 
  ...
  left_join(species_names, by = c("taxon"  = "name_id"))

missing_occ_tsc %>% reactable::reactable(filterable = TRUE)
```

There are `r nrow(missing_occ_tsc)` occurrences missing from tsc when compared to tfa_tae. These missing occurrences come from `r nrow(missing_taxon_tsc_occ)` 381 unique species
