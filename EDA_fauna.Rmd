...
post_one <- . %>% as.list() %>% wastdr::wastd_POST("occ-observation", api_url = dev)
tfa_phys_sample[1,] %>% post_one

# Base R: do several (subset or all)
# apply with margin 1 (rows) does what tidyverse refuses to do
apply(tfa_phys_sample[1:10,], 1, post_one)

# Generalise helper
# Let all params be user defined
wastd_post_one <- function(data_row, serializer, api_url = dev){
  data_row %>% 
    as.list() %>% 
    wastdr::wastd_POST(
...
wastd_bulk_post <- function(data, serializer, api_url){
  apply(data,
        1,
        wastd_post_one,
        serializer = serializer,
...
tfa_phys_sample %>% wastd_bulk_post("occ-observation", api_url = dev)
```
