...
Check unique number of species by both species code and scientific name. 
...
tfa_catcode_unique<-unique(tfa_data$category_cache) #41
```

There are 3063 unique observations in TFL fauna dataset,  
...
`r max(as.date(x$last_date_effective_from, '%m/%d/%y',data = tfa_data))`
`r tfa_data %>% filter(max(last_date_effective_from))`
