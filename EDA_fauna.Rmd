...
length(unique(tsc_occ_fauna$source_id)) # 102563 on Thur 21st, now 102366 on Fri 22nd

## Check for duplicates
tsc_occ_fauna_duplicate_source_ids <- tsc_occ_fauna %>%
  filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])])) #3797 on Fri 22nd

#### Identify missing tae in tsc\_tae
tfa_tae_create <- tfa_tae %>% 
  dplyr::anti_join(tsc_occ_fauna, by = "source_id") #2448

tfa_tae_update <- tfa_tae %>% 
  dplyr::semi_join(tsc_occ_fauna, by = "source_id") #42343
...
tfa_tae_default_datetime <- tfa_tae %>% 
  filter(encountered_on == "1900-01-01 00:00:00+08:00") %>% 
  rt
```
