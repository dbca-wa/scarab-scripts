...
wbp <- function(data, obstype = "PhysicalSample") {
  data %>%
    wastdr::wastd_POST(
...
# Example: one record
tfa_phys_sample_res <- tfa_phys_sample %>% wbp(obstype="PhysicalSample")
tfa_anim_obs[1,] %>% wbp

# Bulk upload
# LONG RUNNING UPLOAD - RE-RUNS CREATE DUPLICATES

tfa_anim_obs_res <- tfa_anim_obs[1,] %>%  wbp(obstype="AnimalObservation")

# We cannot map hand-written AssSpecies to TSC NameIDs.
# tfa_ass_species_res <- tfa_ass_species[1,] %>% wbp(obstype="AssociatedSpecies")

tfa_veg_class_res <- tfa_veg_class %>% wbp(obstype="VegetationClassification")

tfa_hab_comp_res <- tfa_hab_comp %>% wbp(obstype="HabitatComposition") # fails with 400
