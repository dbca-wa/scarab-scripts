...
tsc_occ_fauna <- tsc_tae%>% 
  ...
  dplyr::right_join(tsc_taxa_fauna, by="taxon") %>% na.omit()
...
tfa_occ_tally_top <- tfa_occ_tally %>% 
  top_n(20,n)

tfa_occ_tally_btm <- tfa_occ_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) 
...
tae_tally_btm <- tae_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) # There are actually 151 species with an occurence of 1

missing_species <- tae_tally %>% anti_join(tfa_occ_tally,by = "name_id")
...
One species has different numbers of occurrences between tfa_occ and tfa_tae.
...
```
The two occurences above do not appear in tfa_tae because there is no lat or long data, 
which is a requirement for valid taxon area encounters.

### TAE occurences not in TSC vv
```{r tally_tfa_occ}
tae_tally <- tfa_tae %>%
  dplyr::group_by(taxon) %>% 
  dplyr::tally() %>%
...
missing_species_tae_tsc <- tae_tally %>% anti_join(tsc_tally,by = "taxon") # no missing species
