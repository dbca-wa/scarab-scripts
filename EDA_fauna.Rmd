...
```{r extract_tsc_occurrences, message=FALSE}
...
tsc_occ_fauna <- tsc_tae %>%
  ...
  dplyr::right_join(tsc_taxa_fauna, by="taxon")
...
**NOTE** In tfa_tae `Ninox connivens` and `Ixobrychus flavicollis australis` 
are their combined north and south subpops from tfa_occ.
...
## all differences
missing_occurrences <- tfa_tae_tally %>%
...
```
There are `r nrow(missing_species_tae_tsc)` species missing from tfl_tae when compared to tsc_occ. 
