...
default_date <- lubridate::parse_date_time("1900-01-01 00:00:00",
                                           orders = "ymd HMS",
                                           tz = tz)
tfa_raw <- tfa_data$`Fauna Records` 

# taxon occurances
tfa_occ <- tfa_raw %>% dplyr::transmute(
    id = DBNo %>% as.integer(),
...
tfa_tae %>% dplyr::group_by(taxon) %>% dplyr::tally() %>% dplyr::arrange(-n) %>% filter(n>400)
```
...
eoo_24557 <- eoo_polygon(tfa_occ, nid = 24557)
mapview::mapview(eoo_24557)

anim_fauna(tfa_occ, "Malleefowl", 24557)
```
...
eoo_24734 <- eoo_polygon(tfa_occ, nid = 24734)
mapview::mapview(eoo_24734)

anim_fauna(tfa_occ, "Carnaby's Black Cockatoo", 24734)
```
...
**Note** Only occurrence records with valid NameIDs and coordinates are uploaded.

```{r prepare eoo}
# extract all eoo
fna <- tfa_occ %>% dplyr::filter(!is.na(latitude)) %>% dplyr::filter(!is.na(longitude))
...
tfa_listing_cat_iucn2012 %>% anti_join(tsc_listing_cat_iucn2012,by = "taxon") %>% 
...
tfa_listing_cat_iucn2001 %>% anti_join(tsc_listing_cat_iucn2001,by = "taxon") %>% 
  reactable::reactable(filterable = T)
...
tfa_listing_cat_iucn1994 %>% anti_join(tsc_listing_cat_iucn1994,by = "taxon") %>% 
  reactable::reactable(filterable = T)
...
tfa_listing_cat_wawca %>% anti_join(tsc_listing_cat_wawca,by = "taxon") %>% 
  reactable::reactable(filterable = T)
...
tfa_listing_cat_waps %>% anti_join(tsc_listing_cat_waps,by = "taxon") %>% 
  reactable::reactable(filterable = T)
...
tfa_listing_cat_spfn %>% anti_join(tsc_listing_cat_spfn,by = "taxon") %>% 
  reactable::reactable(filterable = T)
...
tfa_listing_cat_epbc %>% anti_join(tsc_listing_cat_epbc,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
...
```{r }
#### IUCN 2012
tsc_listing_cat_iucn2012 %>% anti_join(tfa_listing_cat_iucn2012,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### IUCN 2001
tsc_listing_cat_iucn2001 %>% anti_join(tfa_listing_cat_iucn2001,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### IUCN 1994
tsc_listing_cat_iucn1994 %>% anti_join(tfa_listing_cat_iucn1994,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### WAWCA
tsc_listing_cat_wawca %>% anti_join(tfa_listing_cat_wawca,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### WAPS
tsc_listing_cat_waps %>% anti_join(tfa_listing_cat_waps,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### SPFN
tsc_listing_cat_spfn %>% anti_join(tfa_listing_cat_spfn,by = "taxon") %>% 
  reactable::reactable(filterable = T)

#### EPBC
tsc_listing_cat_epbc %>% anti_join(tfa_listing_cat_epbc,by = "taxon") %>% 
  reactable::reactable(filterable = T)
```
...
The table `tfa_occ` contains species occurrence records with information about:
...
There are `r nrow(tfa_occ)` species occurrence records (encounters). 
...
tfa_occ_no_nameid <- tfa_occ %>% 
    ...
    unique()
tfa_occ_no_nameid %>% kable
```

There are `r nrow(tfa_occ_no_nameid)` distinct scientific fauna names without NameID.
...
tfa_tally %>% head(n = 20) %>% kable(.)
```

There are `r nrow(fauna_tally)` fauna taxa

### TSC occurances not in TFA
