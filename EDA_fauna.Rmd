...
# Example: one record
tfa_phys_sample[1,] %>% wastdr::wastd_bulk_post("occ-observation")

# Bulk upload
# LONG RUNNING UPLOAD
tfa_phys_sample %>% wastdr::wastd_bulk_post("occ-observation")
tfa_anim_obs %>% wastdr::wastd_bulk_post("occ-observation")
tfa_ass_species %>% wastdr::wastd_bulk_post("occ-observation")
tfa_veg_class %>% wastdr::wastd_bulk_post("occ-observation")
tfa_hab_comp %>% wastdr::wastd_bulk_post("occ-observation")
```
...
tsc_occ_fauna <- tsc_tae %>% 
  ...
  dplyr::inner_join(tsc_taxa_fauna, by = c("taxon")) %>% na.omit("source_id")
...
Any records in the following table indicate occurrences in TFA that were not 
migrated into TSC.
...
compared to tfa\_occ. The occurrences listed were likely not migrated because
they do not contain lat or long data which is a requirement for valid taxon area
encounters.

The table below is a tally of mismatching occurrences and confirms the two
...
Any records in the following table indicate taxon in TFA that were not migrated
into TSC.

#### Name mismatches tfa\_tae to tsc\_tae
...
There are `r nrow(missing_taxa_tsc)` taxon records missing from `tsc_tae` when 
...
```{r show_name_mismatches_tfa_tae_to_tsc_occ_flora}
...
missing_taxa_occ_fauna_unique %>% rt
```

There are `r nrow(missing_taxa_occ_fauna)` taxon records missing from 
`tsc_occ_fauna` when compared to `taf_tae`. These records are attributed to 
`r nrow(missing_taxa_occ_fauna_unique)` unique species.  
...
There are `r nrow(missing_occ_tae_tsc)` unique occurrences missing from
`tsc_tae` when compared to `tfa_tae`. These missing occurrences come from 
`r nrow(missing_occ_tae_tsc_unique)` unique species.

The table below is a tally of mismatching occurrences and confirms the missing
occurrences identified above.

```{r show_tally_tfa_tsc_tae}
tfa_tae_tally <- tfa_tae %>%
...
#### Occurrence mismatches tfa_\_tae to tsc\_occ\_fauna

```{r show_missing_occurrences_tfa_tae_to_tsc_occ_fauna}
...
There are `r nrow(missing_occ_tfa_occ_fauna)` unique occurrences missing from 
`tsc_occ_fauna` when compared to `tfa_tae`. These missing occurrences come from 
`r nrow(missing_occ_tfa_occ_fauna_unique)` unique species.

The table below is a tally of mismatching occurrences and confirms the missing 
occurrences identified above.

```{r show_tally_tfa_tae_occ_fauna}
tfa_tae_tally <- tfa_tae %>%
  dplyr::group_by(source_id) %>% 
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>% 
