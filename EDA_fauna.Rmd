...
# Context
Extract TFA data from old database and conservation listing categories and their criteria from TSC.

Load TFA data and manually mapped criteria to TSC and download data from TSC
...
```{r conslist}
tsc_cons_list <- wastdr::wastd_GET("conservationlist", api_url = prod) %>% 
...
```{r conscat}
tsc_cons_cat <- wastdr::wastd_GET("conservationcategory", api_url = prod) %>% 
  ...
  dplyr::left_join(tsc_cons_list, by = "list_id")
```
...
### TFA conservation criteria

```{r tfa_cons_listing_manual}
# ETL condensed criteria mixed with free text, therefor must be manually mapped
...
`r all_crit_map %>% nrow` conservation listings contain condensed 
lists of criteria mixed with free text. Manual mapping required
...
```{r}
tfa_cons_list_w_tsc %>% dplyr::filter(name_id == 0)
```

Let's save a snapshot of the cons listings.

```{r tfa_cons_listing_save}
tfa_tsc_file <- here::here("data", "tfa_cons_list_w_tsc.Rdata")
save(tfa_cons_list_w_tsc, file=tfa_tsc_file)
if (file.exists(tfa_tsc_file)) load(tfa_tsc_file)
```
### TFA conservation listing upload
...
# IUCN(2001) 4
tfa_listing_cat_iucn2001 <- fauna_cons_listing_w_tsc %>% 
...
# IUCN(1994) 5
tfa_listing_cat_iucn1994 <- fauna_cons_listing_w_tsc %>% 
...
# WAWCA 1
tfa_listing_cat_wawca <- fauna_cons_listing_w_tsc %>% 
...
# SPFN 2
tfa_listing_cat_spfn <- fauna_cons_listing_w_tsc %>% 
...
# WAPS 6
tfa_listing_cat_waps <- fauna_cons_listing_w_tsc %>% 
...
# EPBC 7
tfa_listing_cat_epbc <- fauna_cons_listing_w_tsc %>% 
...
tsc_data$name_id <- as.character(tsc_data$name_id)
```
...
tfa_data <- tfa_cons_list_w_tsc %>% 
  ...
  unite("category_cache", list_code:category_code, sep = " ", remove = FALSE)

tfa_unique<-unique(tfa_data$name_id) #870
tfa_catcode_unique<-unique(tfa_data$category_cache) #40
```

There are 4014 unique observations in TFL fauna dataset,  
`r unique(tfa_data$species_code)` unique species and 
`r unique(tfa_data$category_cache)` conservation category codes.

```{r tsc_summary}
tsc_unique<-unique(tsc_data$taxon) #870
tsc_catcode_unique<-unique(tsc_data$category_cache) #40
```

There are 3061 unique observations in TSC fauna dataset,  
`r unique(tfa_data$species_code)` unique species and 
`r unique(tfa_data$category_cache)` unique conservation category codes.
...
tfa_data_wacensus <- tfa_cons_list_w_tsc %>% dplyr::left_join(wacensus_names, by="name_id")
```
