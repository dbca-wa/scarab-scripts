...
The EOO (Extent of occurence) is calculated as the convex hull around all recorded fauna occurences, including fossil records.
...
tfa_tae_tally_top <- tfa_tae_tally %>% 
  top_n(20,n)

tfa_tae_tally_btm <- tfa_tae_tally %>% 
  top_n(20,-n) %>% 
  slice(1:20) # There are actually 151 species with an occurence of 1
...
missing_species <- tfa_tae_tally %>% anti_join(tsc_tally,by = "taxon") # no missing species

tfa_tae_tsc_top_diff <- tfa_tae_tally_top %>%
  dplyr::rename(tfa_tae = n) %>% 
  left_join(tsc_tally_top,by = "taxon") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tfa_tae != tsc)

tfa_tae_tsc_btm_diff <- tfa_tae_tally_btm %>%
  dplyr::rename(tfa_tae = n) %>% 
  left_join(tsc_tally_btm,by = "taxon") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tfa_tae != tsc)
