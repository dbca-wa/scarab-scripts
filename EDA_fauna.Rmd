...
species_notes_dup <- species_notes %>%
  filter(species_code %in% unique(.[["species_code"]][duplicated(.[["species_code"]])])) # two duplicate species_codes
...
# The above needs to be added back in after we deal with duplicates
species_dup <- species %>%
  filter(name_id %in% unique(.[["name_id"]][duplicated(.[["name_id"]])])) # three duplicate name_id
...
```{r}
missing_tae_phys_sample <- tfa_phys_sample %>% 
  dplyr::anti_join(tfl_tae, by = "source_id")
missing_tae_anim_obs <- tfa_anim_obs %>% 
  dplyr::anti_join(tfl_tae, by = "source_id")
missing_tae_veg_class <- tfa_veg_class %>% 
  dplyr::anti_join(tfl_tae, by = "source_id")
missing_tae_hab_comp <- tfa_hab_comp %>% 
  dplyr::anti_join(tfl_tae, by = "source_id")
...
The following tables show any occurrence observations in TFL which are not in TSC,
for each observation table.

Physical Sample

```{r show_tfl_tcl_not_in_tsc_iucn2012}
tsc_phys_sample <- tsc_tae_observations %>% 
  dplyr::filter(obstype == "PhysicalSample")

tfl_phys_sample %>%
  dplyr::anti_join(tsc_phys_sample, by = "source_id") %>%
  rt
```
...
tfl_hab_cond %>%
  dplyr::anti_join(tsc_hab_cond, by = c("source_id" = "id")) %>% 
  rt
```
...
tfl_area_ass %>%
  dplyr::anti_join(tsc_area_ass, by = c("source_id" = "id")) %>% 
  rt
```
...
tfl_hab_comp %>%
  dplyr::anti_join(tsc_hab_comp, by = c("source_id" = "id")) %>%  
  rt
```
...
```{r show_tfl_tcl_not_in_tsc_prio}
tfl_plant_count %>%
  dplyr::anti_join(tsc_plant_count, by = c("source_id" = "id")) %>%  
  rt
```
