...
# if (file.exists(tfa_file)) load(tfa_file)
...
occurence data is extracted and transformed. 
...
# This works in DEV - coming soon to PROD TSCs in your area!
# tfa_phys_sample[1,] %>% as.list() %>% wastdr::wastd_POST("occ-observation", api_url = dev)
```
...
tfa_tae %>% chunk_post(serializer = "occ-taxon-points", verbose = T, api_url = prod) # upload for all occurences
```


### Upload ObservationGroups

TODO upload obsgroups here.
...
tsc_occ_fauna <- tsc_tae%>% 
  ...
  dplyr::right_join(tsc_taxa_fauna, by = c("taxon")) %>% na.omit("source_id")
...
missing_occ_tae <- tfa_occ %>%
  dplyr::anti_join(tfa_tae, by = c("id" = "source_id")) 

missing_occ_tae %>% reactable::reactable(filterable = TRUE)

```

There are `r nrow(missing_occurrences)` occurrences missing from tfa_tae when compared to tfa_occ. The occurrences listed were likely not migrated because they do not contain lat or long data which is a requirement for valid taxon area encounters.
...
### TAE occurrences not in TSC

Any records in the following table indicate occurrences in TFA that were not migrated into TSC.

#### Name mismatches 
```{r}
missing_taxa_tsc_tae <- tfa_tae %>% 
  select(taxon) %>% 
  left_join(tsc_tae, by = "taxon")
  
  setdiff(tfa_tae$taxon,tsc_tae$taxon) %>% as.data.frame() %>% 
  rename(. = taxon)

missing_taxa_tsc <- mutate(taxon == setdiff(tfa_tae$taxon,tsc_occ_fauna$taxon) %>% as.data.frame())
...
```{r missing_occurrences}
unique(tfa_tae$taxon) #892
unique(tsc_tae$taxon) #871
unique(tsc_occ_fauna$taxon) #745

intersect(tfa_tae$taxon,tsc_tae$taxon) #871
setdiff(tsc_tae$taxon,tfa_tae$taxon) # 0 taxon in tsc that are not in tfa
setdiff(tfa_tae$taxon,tsc_tae$taxon) # 26 taxon in tfa that are not in tsc

intersect(tsc_occ_fauna$taxon,tfa_tae$taxon) # 745 similar
setdiff(tsc_occ_fauna$taxon,tfa_tae$taxon) # 0 taxon in tsc that are not in tfa
setdiff(tfa_tae$taxon,tsc_occ_fauna$taxon) # 152 taxon in tfa that are not in tsc

unique(tsc_occ_fauna$source_id) #55744
unique(tfa_tae$source_id) #94814

intersect(tsc_occ_fauna$source_id,tfa_tae$source_id) #55744
setdiff(tsc_occ_fauna$source_id,tfa_tae$source_id) # 0 source_id's in tsc that are not in tfa
setdiff(tfa_tae$source_id,tsc_occ_fauna$source_id) # 39070 source_id's in tfa that are not in tsc




missing_occ_tsc <- tfa_tae %>%
  dplyr::anti_join(tsc_occ_fauna, by = "source_id") 


missing_taxon_tsc_occ <- missing_occ_tsc %>% 
  dplyr::select(taxon) %>% distinct() %>% 
  left_join(tfa_tae, by = c("taxon"  = "name_id"))

missing_occ_tsc %>% reactable::reactable(filterable = TRUE)
```

There are `r nrow(missing_occ_tsc)` occurrences missing from tsc when compared to tfa_tae. These missing occurrences come from `r nrow(missing_taxon_tsc_occ)` 385 unique species

The occurrences listed were likely not migrated because ????????
...
tae_tsc_tally <- tae_tally %>%
  dplyr::rename(tae = n) %>% 
  left_join(tsc_occ_tally,by = "source_id") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tae != tsc) # this isn't correct
```

There are  `r nrow(tae_tsc_tally)` mismatching occurrences between tfa_tae and tsc.

START HERE

```{r} 
## all differences
missing_occurrences <- tae_tally %>%
  dplyr::rename(tfa_tae = n) %>% 
  left_join(tsc_occ_tally,by = "taxon") %>% 
  dplyr::rename(tsc = n) %>% 
  filter(tfa_tae != tsc) %>% glimpse() # 229

missing_occurrences_many <- missing_occurrences %>% 
  dplyr::filter(tsc == 1) # 18 taxon missing which have multiple occurrences

missing_occurrences_one <- missing_occurrences %>% 
  dplyr::filter(tsc != 1) # 211 taxon missing which have only one occurrence

# tfa_tae_tsc_diff_one <- tfa_tae_tsc_diff %>% 
#   dplyr::filter.sf(geometry == NA) ## How do you filter a geometry column???
```
There are `r nrow(missing_occurrences)` occurrences missing from tsc when compared to tae_occ. 

Where there is no geometry data the occurrences of that species have not been carried across. 

Where differences between tfa_tae and tsc are greater than 1, something bigger is going on ad a thorough review is required.
