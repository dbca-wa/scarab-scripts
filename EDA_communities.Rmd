...
```{r load_tec_cons, warning=FALSE,message=FALSE}
...
tec_land_element <- here::here("csv/tec_landforms_w_tsc.csv") %>%
  readr::read_csv(
    col_types = cols(
...
tec_hab_comp <- hab_comp %>%
  left_join(tec_land_element, by = c("land_element")) %>%
  left_join(tec_drainage, by = c("drainage")) %>%  
dplyr::select(source,occ_id,tsc_landform,tsc_drainage) %>% 
dplyr::rename(source_id = occ_id) %>%
dplyr::rename(landform = tsc_landform) %>% 
...
```{r, eval=FALSE}
tec_hab_comp[1,] %>% wastd_bulk_post("occ-observation", verbose = TRUE)

tec_hab_comp %>% wastd_bulk_post("occ-observation")
...
datafile_tsc_community <- here::here("data/community_tsc.RData")
if (file.exists(datafile_tsc_community)) {
  load(datafile_tsc_community)
} else {
# Taxon conservation listings for communities
  tsc_ccl_communities <- wastdr::wastd_GET("community-conservationlisting") %>%
  wastdr::parse_community_conservationlisting() %>% 
    dplyr::mutate(com_label = community)
   
save(tsc_ccl_communities, file = datafile_tsc_community, compress="xz")
}
```

## Occurrences

We get community occurrences with their point and boundary representations from TSC through the 
API endpoint [occ-community-points] (https://tsc.dbca.wa.gov.au/api/1/occ-community-points)
[occ-community-areas] (https://tsc.dbca.wa.gov.au/api/1/occ-community-areas/).

```{r extract_tsc_occurrences, message = FALSE, eval = FALSE}
datafile.community <- here::here("data/community_cae.RData")

if (file.exists(datafile.community)) {
  load(datafile.community)
} else {
tsc_cae_data <- wastdr::wastd_GET("occ-area-points") # k records
tsc_cae_data <- wastdr::wastd_GET("occ-area-points", max_records = 10)
...
save(tsc_cae_data, tsc_community_sites, tsc_community_boundaries, file=datafile.community, compress="xz")
}
```
...
#
tsc_ccl_communities <- tsc_ccl_communities %>% dplyr::mutate(com_label = community)
