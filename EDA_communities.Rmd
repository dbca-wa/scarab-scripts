...
tsc_extracted <- here::here("data/tsc_extracted_communities.RData")
```
...
save(tsc_ccl_communities,
     ...
     tsc_tae_observations_communities,
...
unique(tsc_community_sites$source)
length(unique(tsc_community_sites$source_id))

unique(tsc_community_boundaries$source)
length(unique(tsc_community_boundaries$source_id))

## Check for duplicates
tsc_community_sites_duplicate_source_ids <- tsc_community_sites %>%
  filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])])) 
tsc_community_boundaries_duplicate_source_ids <- tsc_community_boundaries %>%
  filter(source_id %in% unique(.[["source_id"]][duplicated(.[["source_id"]])])) 

#### Identify missing tae in tsc\_cae
site_cae_create <- tec_site_cae %>% 
  dplyr::anti_join(tsc_community_sites, by = "source_id")

site_cae_update <- tec_site_cae %>% 
  dplyr::right_join(tsc_community_boundaries, by = "source_id")
...
```{r make_tec_summary}
tec_ccl_unique_comm <- tec_comms_ccl$community %>%
  unique() %>%
  length() # 399

tsc_ccl_unique_comm <- tsc_ccl_communities$community %>%
  unique() %>%
  length() # 399
```

## Conservation Listing
TEC has `r nrow(tec_comms_ccl)` conservation listings of `r tec_ccl_unique_comm` 
communities. TSC has `r nrow(tsc_ccl_communities)` community conservation listings of 
