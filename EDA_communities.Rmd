...
names_tec <- as.data.frame(names(tec))
...
tec_names <- tibble::tibble(names(tec_occ))
...
## surveys data exploration
nrow(surveys) # 4492 surveys
x <- unique(surveys$source_id) # 4088 unique source_id (boundary_id + survey_id), so some boundaries have been surveyed multiple times

## Check for similarities between surveys and boundaries
x <- intersect(surveys$boundary_id, tec_bdy$BDY_ID) # 2247 similar
...
site_visit$site_code %>% head(n = 100)

## Sites and site_visit data exploration
nrow(site_visit) # 172 site visits
unique(site_visit$site_code) # 136 unique site_codes, so some sites have been visited multiple times
nrow(sites) # 204 sites
unique(sites$site_code) # 204 unique site_codes
intersect(sites$site_code, site_visit$site_code) # no common site_codes between sites and site_visit.
# Don't use sites further, use the tec_sit shapefile to identify sites for site_visit
## Check for similarities with site_visits
intersect(site_visit$site_code, tec_sit$S_ID) # 131 similar
...
nrow(sites_keep) # 3981 unique rows
unique(sites_keep$BDY_ID) # 956 unique boundary IDs

intersect(site_visit$site_code, sites_keep$S_ID) # 105 site_visit match site_code in sites_keep
intersect(sites$site_code, sites_keep$S_ID) # 59 sites match site_code in sites_keep
...
## Check for similarities between surveys and boundaries
x <- intersect(tec_occ$boundary_id, tec_bdy$BDY_ID) # 122049 similar
setdiff(tec_bdy$BDY_ID, tec_occ$boundary_id) # There are 0 boundarys in tec_bdy that are not in tec_occ
setdiff(tec_occ$boundary_id, tec_bdy$BDY_ID) # There are 566 boundaries in tec_occ that are not in tec_bdy
setdiff(tec_bdy$BDY_ID, surveys$boundary_id) # There are 118803 boundaries that have never been surveyed
setdiff(surveys$boundary_id, tec_bdy$BDY_ID) # There are 129 surveyed boundaries which don't have a matching boundary ID in tec_bdy

unique(surveys$com_id)
...
names(survey_cae)
setdiff(survey_cae$com_id, communities$com_id) # There are 145 communities that have not been surveyed

tec_bdy_cae <- survey_cae %>%
  dplyr::transmute(
    # occ_id = occ_id,
    community = com_id,
    code = boundary_id,
...
site_visit_cae <- site_visit %>%
  left_join(tec_sit, by = c("site_code" = "S_ID")) %>%
  left_join(tec_occ, by = c("OCC_UNIQUE" = "occ_id")) %>%
  left_join(communities, by = c("com_id", "com_label", "com_name"))

tec_sit_cae <- site_visit_cae %>%
  dplyr::transmute(
    # occ_id = OCC_UNIQUE,
    community = COM_ID,
    code = site_code,
    name = site_code,
    description = glue::glue("Site ID {site_code} inside TEC Boundary {BDY_ID}"),
...
## The following communities contain no com_id and therefore cannot be parsed
no_com_id <- tec_sit_cae %>%
  dplyr::filter(is.na(community))

## Filter out the NA's
tec_sit_cae <- tec_sit_cae %>%
  dplyr::filter(community != "NA")

no_com_id %>% reactable::reactable()
```

The two communities above do not contain a community ID so they cannot be parsed to TSC
...
# Extract and transform occurrence lookup Data from TSC and create final data frames
...
This section lists data issues which need to be fixed at the source (here, in legacy TEC).

The date column for surveys `surveyed_on` and site_visits `site_visit_date` is required.

Where a time is missing, we'll default to midday (12:00:00).
...
* A safe default of the observation time (midday) is an appropriate approximate time for surveys, which are carried out during daylight hours.
...
* `r surveys %>% dplyr::filter(surveyed_on == "") %>% nrow()` date_encountered_on couldn't be restored and require a valid date (and ideally time) to be set at the source.
* `r site_visit %>% dplyr::filter(is.na(site_visit_date)) %>% nrow()` date_encountered_on couldn't be restored and require a valid date (and ideally time) to be set at the source.

### Surveys: default/ missing dates
```{r missing dates}
surveys_no_date <- surveys %>% 
    filter(surveyed_on == "")
#surveys_no_date %>% reactable::reactable()
```
There are `r nrow(surveys_no_date)` distinct scientific fauna names missing a date.

### Sites: default/ missing times
```{r missing times}
site_visit_no_date <- site_visit %>% 
    filter(is.na(site_visit_date))
site_visit_no_date %>% reactable::reactable()
```
There are `r nrow(site_visit_no_date)` distinct scientific fauna names missing a time.

### Missing boundary ID
```{r missing boundary ID}
tec_bdy_missing_boundary_ID <- tec_bdy %>% 
    dplyr::filter(BDY_ID == 0)
#tec_bdy_missing_boundary_ID %>% reactable::reactable()
```

There are `r nrow(tec_bdy_missing_boundary_ID)` distinct boundary records with a missing boundary ID.

### Missing site visit ID
```{r missing site visit ID}
names(tec_sit)
tec_occ_no_sit <- tec_sit %>% 
    dplyr::filter(S_ID == 0)
#tec_occ_no_sit %>% reactable::reactable()
```

There are `r nrow(tec_occ_no_sit)` distinct site records with a missing site ID.
...
#   resource_id = "9483f333-97fd-4975-a799-d44bf8bd252a"
# )
# upload_to_ckan(publications, "TEC Publications",
#   d$id,
#   resource_id = "9483f333-97fd-4975-a799-d44bf8bd252a"
# )
# upload_to_ckan(recommendations, "TEC Recommendations",
#   d$id,
#   resource_id = "9483f333-97fd-4975-a799-d44bf8bd252a"
# )
# upload_to_ckan(reviews, "TEC Reviews",
#   d$id,
#   resource_id = "9483f333-97fd-4975-a799-d44bf8bd252a"
# )
