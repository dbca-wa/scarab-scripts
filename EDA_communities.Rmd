...
if (file.exists(tec_extracted)) {
  ...
  tec_bdy_raw <- geojsonsf::geojson_sf("data/tec_boundaries.geojson")
  tec_site_raw <- geojsonsf::geojson_sf("data/tec_sites.geojson")
  # tec_buf <- geojsonsf::geojson_sf("data/tec_buffers.geojson")
  # tec_app <- dl_mdbzip("27d9b026-6933-4516-acf6-41edc6134adb")

save(tec_data, snapshot_last_updated, tec_bdy_raw, tec_site_raw, file=tec_extracted)
}
```
...
the `tec_bdy_raw` shapefile of last known extents of boundaries.
...
TEC boundaries store the first site name as `tec_bdy_raw$FIRST_S_ID`.
CAE need to use `tec_bdy_raw$FIRST_S_ID` as `area_code`.
...
-   `r tec_bdy_cae %>% dplyr::filter(encountered_on == "") %>% nrow()` boundary 
    survey records lack an explicit date\time and were defaulted to midnight AWST.
-   `r tec_site_cae %>% dplyr::filter(encountered_on == "") %>% nrow()` site
...
-   `r tec_occ_no_sit <- tec_site_cae %>% dplyr::filter(S_ID == 0) %>% nrow()` site
