...
tec_extracted <- here::here("data", "tec_extracted.Rdata")
tec_transformed <- here::here("data/tec_transformed.RData")
tec_occ_datafile <- here::here("data", "tec_occ.Rdata")
tsc_extracted_communities <- here::here("data/tsc_extracted_communities.RData")
```
...
In this section, data from the Threatened Ecological Communities Database (TEC) is downloaded from the 
...
This section extracts communties data from TEC. 
This data will later be compared to the taxonomic data from TSC.

```{r tec_extract}
if (file.exists(tec_extracted)) {
  load(tec_extracted)
} else {
  # TEC data
  tec_data <- dl_mdbzip("598b1e70-5707-4dbc-a665-1189e1524ebe")
  
  snapshot_last_updated <- 
    "598b1e70-5707-4dbc-a665-1189e1524ebe" %>%
    ckanr::resource_show() %>%
    magrittr::extract2("last_modified") %>%
    lubridate::as_datetime() %>%
    lubridate::with_tz("Australia/Perth")
  
  # TEC boundaries and sites
...
save(tec_data, snapshot_last_updated, tec_bdy, tec_site, file=tec_extracted)
}
```
...
```{r tec_transformed}
# Look up tables
former_range <- tec_data$FORMER_RANGES %>%
  dplyr::transmute(
...
readr::write_csv(
  former_range, 
  path = here::here("data", "tec_former_range.csv"))

range_decline <- tec_data$RANGE_DECLINE %>%
  dplyr::transmute(
...
readr::write_csv(
  range_decline, 
  path = here::here("data", "tec_range_decline.csv"))

occ_decline <- tec_data$OCCURRENCES_DECLINE %>%
  dplyr::transmute(
...
readr::write_csv(
  occ_decline, 
  path = here::here("data", "tec_occ_decline.csv"))

status <- tec_data$STATUS %>%
  dplyr::transmute(
...
readr::write_csv(
  status, 
  path = here::here("data", "tec_status.csv"))

communities <- tec_data$COMMUNITIES %>%
  tibble::as_tibble() %>%
  dplyr::transmute(
...
recommendations <- tec_data$RECOMMENDATIONS %>%
  dplyr::transmute(
    recommendation_code = REC.CODE %>% as.character(),
    recommendation = REC.DESC %>% as.character()
  )

com_recommendations <- tec_data$COMMUNITY_RECOMMENDATIONS %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
readr::write_csv(
  com_recommendations, 
  path = here::here("data", "tec_com_recommendations.csv"))
```

### Community Publications

```{r make_com_publications}
publications <- tec_data$PUBLICATIONS %>%
  dplyr::transmute(
...
com_publications <- tec_data$COMMUNITY_PUBLICATIONS %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
readr::write_csv(
  com_publications, 
  path = here::here("data", "tec_com_publications.csv"))
```

### Community Reviews

```{r make_com_reviews}
reviews <- tec_data$REVIEWS %>%
  dplyr::transmute(
...
com_reviews <- tec_data$COMMUNITY_REVIEWS %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
readr::write_csv(
  com_reviews, 
  path = here::here("data", "tec_com_reviews.csv"))
```

### Community Actions

```{r make_com_actions}
actions <- tec_data$ACTIONS %>%
  dplyr::transmute(
    action_id = ACTION.CODE %>% as.character(),
    description = ACTION.DESC %>% as.character()
  )

com_actions <- tec_data$COMMUNITY_ACTIONS %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
readr::write_csv(
  com_actions, 
  path = here::here("data", "tec_com_actions.csv"))
```

### Community History, Area Decline

```{r make_com_history}
area_decline <- tec_data$AREA_DECLINE %>%
  dplyr::transmute(
    area_decline_id = AD.CODE %>% as.integer(),
    area_decline = AD.DESC %>% as.character()
  )

com_history <- tec_data$HISTORY %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
This section extracts conservation listings from TEC.
TEC conservation criteria are exported as CSV spreadsheet for manual mapping
to TSC values.

```{r make_tec_ccl}
tec_cons_cat <- tec_data$CATEGORY_TYPES %>%
  dplyr::transmute(
...
tec_cons_crit <- tec_data$CRITERIA_CODES %>%
  dplyr::transmute(
...
readr::write_csv(
  tec_cons_crit, 
  path = here::here("data", "tec_cons_crit.csv"))

# Using a built-in function to turn uppercase into lowercase? Nah. Hold my beer.
authorities <- tec_data$AUTHORITIES # A lookup for upper cased authorities vs lower cased authorities.
endorsement <- tec_data$ENDORSED_BY # another lookup for lower case.
readr::write_csv(
  endorsement, 
  path = here::here("data", "tec_endorsement.csv"))

# primary
tec_cons_listing_categories <- tec_data$CATEGORIES %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
    effective_from = CAT.EFFECT.DATE %>% parse_date_time(., orders = orders, tz = tz),
    category_code = CAT.CT.TYPE %>% as.character(),
    cat_comment = CAT.COMMENT %>% as.character(),
    cat_review_date = CAT.REVIEW.DATE %>% parse_date_time(., orders = orders, tz = tz),
    cat_endorsed_code = CAT.ENDORSED.CODE %>% as.character(),
    cat_endorsed_date = CAT.ENDORSED.DATE %>% parse_date_time(., orders = orders, tz = tz),
...
tec_cons_listing_criteria <- tec_data$CATEGORY_CRITERIA %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
    effective_from = CAT.EFFECT.DATE %>% parse_date_time(., orders = orders, tz = tz),
...
readr::write_csv(
  all_crit_map, 
  path = here::here("data", "all_tec_conscrit.csv"))
```

## Occurrences
This section extracts TEC occurrences.

```{r get_tec_occ, message = FALSE, warning = FALSE}
reliability <- tec_data$RELIABILITY %>% dplyr::transmute(
  reliability_id = BR.CODE %>% as.character(),
  reliability = BR.DESC %>% as.character()
)
readr::write_csv(
  reliability, 
  path = here::here("data", "tec_reliability.csv"))

tec_occ <- tec_data$OCCURRENCES %>% ## most of this will get added to comments
  tibble::as_tibble() %>%
  dplyr::transmute(
    # IDs
...
```{r make_occ_actions}
occ_actions <- tec_data$OCCURRENCE_ACTIONS %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
occ_recommendations <- tec_data$OCCURRENCE_RECOMMENDATIONS %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
readr::write_csv(
  sources, 
  path = here::here("data", "tec_sources.csv"))

occ_extent <- tec_data$EXTENTS %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
readr::write_csv(
  items, 
  path = here::here("data", "tec_items.csv"))

occ_additional_data <- tec_data$ADDITIONAL_DATA %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
occ_maps <- tec_data$OCCURRENCE_MAPS
...
occ_biol_process <- tec_data$BIOL_PROCESSES %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
occ_nonbiol_process <- tec_data$NON_BIOL_PROCESSES %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
fire_history <- tec_data$FIRE_HISTORY %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
```{r make_com_occ}
# lookup tables
fauna_conservation_codes <- tec_data$FAUNA_CONSERVATION_CODES %>%
  dplyr::transmute(
    conservation_code = CONSV.CODE %>% as.character(),
    conservation_code_description = CONSV.DESC %>% as.character()
  )

fauna_groups <- tec_data$FAUNA_GROUPS # paraphyletic groups

species_roles <- tec_data$SPECIES_ROLES %>%
  dplyr::transmute(
...
readr::write_csv(
  species_roles, 
  path = here::here("data", "tec_species_roles.csv"))

# primary data
occ_fauna <- tec_data$OCCURRENCE_FAUNA %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
occ_species <- tec_data$OCCURRENCE_SPECIES %>%
  dplyr::transmute(
...
# lookups
impacts <- tec_data$THREAT_IMPACTS %>%
  dplyr::transmute(
...
readr::write_csv(
  impacts, 
  path = here::here("data", "tec_impacts.csv"))

threats <- tec_data$THREATS %>%
  dplyr::transmute(
...
readr::write_csv(
  threats, 
  path = here::here("data", "tec_threats.csv"))

conditions <- tec_data$CONDITIONS %>%
  dplyr::transmute(
...
readr::write_csv(
  conditions, 
  path = here::here("data", "tec_conditions.csv"))

# primary
com_threats <- tec_data$COMMUNITY_THREATS %>%
  dplyr::transmute(
    com_id = COM.NO %>% as.integer(),
...
readr::write_csv(
  surveys, 
  path = here::here("data", "tec_surveys.csv"))

survey_threats <- tec_data$SURVEY_THREATS %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
    survey_id = SUR.NO %>% as.integer(),
...
survey_conditions <- tec_data$SURVEY_CONDITIONS %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
    survey_id = SUR.NO %>% as.integer(),
...
readr::write_csv(
  survey_conditions, 
  path = here::here("data", "tec_survey_conditions.csv"))
```

Out of `r nrow(surveys)` surveys, there are only `r length(unique(surveys$survey_id))` distinct survey_id's.
With `r length(unique(surveys$surveyed_on))` unique survey dates, this means that a 
survey_id does not denote an individual visit of a TEC boundary.
...
readr::write_csv(
  datum, 
  path = here::here("data", "tec_datum.csv"))

sites <- tec_data$SITES %>%
  dplyr::transmute(
    occ_id = OCC.UNIQUE.ID %>% as.integer(),
...
readr::write_csv(
  sites, 
  path = here::here("data", "tec_sites.csv"))

site_visit <- tec_data$SITE_VISITS %>%
  dplyr::transmute(
...
voucher_specimens <- tec_data$SITE_SPECIES %>%
  dplyr::transmute(
...
## Occurrence sites map
The following map shows the occurrence sites data from TEC.
Occurrences without `comm_id` or location are skipped.
...
recovery_plans <- tec_data$INTERIM_RECOVERY_PLANS %>%
  dplyr::transmute(
...
readr::write_csv(
  recovery_plans, 
  path = here::here("data", "tec_recovery_plans.csv"))
```

## Context for `tec_cae`

```{r}
tec_occ_without_bdy <- tec_occ %>% dplyr::filter(is.na(boundary_id))
```

There are `r nrow(tec_occ)` records in `tec_occ`. Most link via `boundary_id` to
the `tec_bdy` shapefile of last known extents of boundaries.

`r nrow(tec_occ_without_bdy)` `tec_occ` do not have a `boundary_id`. 
TODO find spatial reference for those.

`tec_occ$occ_no` is a running number for each spatially distinct occurrence of 
a given community and unique within that community. 
`tec_occ$occ_id` is a running number of all occurrences and unique across
all communities.

Occurrences are named by the first site recorded within.
TEC boundaries store the first site name as `tec_bdy$FIRST_S_ID`.
CAE need to use `tec_bdy$FIRST_S_ID` as `area_code`.
...
tec_bdy_cae_build <- survey_cae %>%
  ...
  dplyr::mutate(description2 = glue::glue(
        ...
        "Habitat composition: {soil} {surf_geology} {land_element} {water} ",
        "{drainage} {com_structure} {classification} {other_attr} {reliability}."
    )) %>% 
  dplyr::transmute(
    # occ_id = occ_id,
    community = com_label,
    ...
    encountered_on = glue::glue("{surveyed_on} 00:00:00+08:00"),
    encountered_by = 1,
    encounter_type = 2,
    geom = geometry %>%
      sf::st_convex_hull(.) %>%
  ...
  dplyr::mutate(
    encountered_on = replace(
      encountered_on, 
      encountered_on == "NA 00:00:00+08:00",
...
tec_site_cae_build <- site_visit_cae %>%
  dplyr::transmute(
    ...
    source = 15, # TEC sit
    source_id = site_visit_id %>% as.character(),
    encounter_type = 2, # https://tsc.dbca.wa.gov.au/admin/occurrence/encountertype/2/change/ = survey
    encountered_on = glue::glue("{site_visit_date} 00:00:00+08:00"),
    encountered_by = 1,
...
save(tec_occ, tec_bdy_cae, tec_site_cae, file=tec_occ_datafile, compress = "xz")
if (file.exists(tec_occ_datafile)) load(tec_occ_datafile)
```

## Save point
```{r savepoint_tec, eval=F}
save(
actions,
all_crit_map,
area_decline,
...
tec_cons_cat,
tec_cons_crit,
tec_cons_cat_mis_crit,
tec_cons_crit_mis_cat,
tec_cons_listing,
tec_cons_listing_categories,
tec_cons_listing_criteria,
tec_bdy_cae,
tec_bdy_cae_build,
tec_occ,
threats,
voucher_specimens,
file = tec_transformed
)
# load(tec_transformed)
```
...
tsc_conslists_com <- "conservationlist" %>%
  tsc_GET(query = c(scope_communities = TRUE)) %>%
...
tsc_cons_crit <-
  tsc_GET("conservationcriterion", api_url = prod) %>%
  magrittr::extract2("features") %>%
  {
    tibble::tibble(
      id = purrr::map_int(., "id"),
...
occurrence data in TEC in a spreadsheet.

```{r get_tsc_occ_lookups}
...
## Soil not manually mapped, 1222 manual mappings required- maybe at a later date!
...
# LONG RUNNING UPLOAD
tec_comms_ccl_res <- tec_comms_ccl %>%
  tsc_POST("community-conservationlisting")
```
...
# LONG RUNNING UPLOAD
tec_bdy_cae_res <- tec_bdy_cae %>%
  filter(grepl(fat_filter, community)) %>%
  tsc_chunk_post(serializer = "occ-community-areas")
```
...
# Upload all occurrences to TSC PROD
# LONG RUNNING UPLOAD
tec_site_cae_res <- tec_site_cae %>%
  dplyr::filter(grepl(fat_filter, community)) %>%
  tsc_chunk_post(serializer = "occ-community-points")
```
...
tec_hab_comp %>% 
  tsc_occ_obs_post(obstype="HabitatComposition", api_url = dev)

tec_hab_comp[1,] %>% 
  wastdr::wastd_occ_obs_post(obstype="HabitatComposition", chunksize = 1000)

# Bulk upload
# LONG RUNNING UPLOAD
tec_hab_comp_res <- tec_hab_comp %>% 
  wastd_occ_obs_post(obstype="HabitatComposition", chunksize = 1000)
```
...
First, we extract conservation listings.

Last, we get community occurrences with their point and boundary representations from TSC through the API endpoint.
...
```{r get_tsc_data}
if (file.exists(tsc_extracted_communities)) {
  load(tsc_extracted_communities)
} else {
  
# Community conservations listings
  tsc_ccl_communities <- "community-conservationlisting" %>% 
    tsc_GET() %>%
    tsc_parse() %>% 
    dplyr::mutate(com_label = community)

# Community occurrences  
 tsc_cae_data <- "occ-community-points" %>% 
  tsc_GET(chunk_size = 100) %>% 
  tsc_parse()
...
tsc_community_boundaries <- tsc_cae_data %>%
 ...
 # Community occurrence observations
  tsc_tae_observations_communities <- "occ-observation" %>% 
    tsc_GET(query = list(source=15)) %>%
    tsc_parse()
  
  tsc_communities_observations <- tsc_tae_observations_communities %>% 
    tidyr::unnest_wider(encounter, names_repair = "universal") %>% 
    tidyr::unnest_wider(properties, names_repair = "universal")  %>% 
    filter(source == 13)
  
  tsc_communities_occ_obs <- tsc_communities_observations %>% 
    dplyr::transmute(
      source = 13,
      source_id = source_id %>% as.character(),
      landform = landform,
...
save(tsc_ccl_communities,
     tsc_cae_data,
     tsc_community_sites,
     tsc_community_boundaries,
     tsc_tae_observations,
     tsc_communities_observations,
     tsc_communities_occ_obs,
     file = tsc_extracted_communities)
}
```

```{r show_duplicates_and_missing_tae, eval = FALSE}
# All sources are communities ()
unique(tsc_community_sites$source) == 15
unique(tsc_community_boundaries$source) == 13

# There are no boundary records in tsc_community_sites
tsc_community_sites %>% dplyr::filter(source == 13) %>% nrow

# There are no site records in tsc_community_boundaries
tsc_community_boundaries %>% dplyr::filter(source == 15) %>% nrow

# There are as many unique source_ids as occurrence records
length(unique(tsc_community_sites$source_id)) == nrow(tsc_community_sites)
length(unique(tsc_community_boundaries$source_id)) == nrow(tsc_community_boundaries)

# There are no source_ids that occur more than once
tsc_site_cae_tally <- tsc_community_sites %>% 
  group_by(source, source_id) %>% 
  dplyr::tally() %>% 
  dplyr::ungroup() %>% 
  dplyr::filter(n>1) %>% 
  dplyr::arrange(-n)
View(tsc_site_cae_tally)

tsc_boundary_cae_tally <- tsc_community_boundaries %>% 
  group_by(source, source_id) %>% 
  dplyr::tally() %>% 
  dplyr::ungroup() %>% 
  dplyr::filter(n>1) %>% 
  dplyr::arrange(-n)
View(tsc_boundary_cae_tally)

#### Identify missing tae in tsc\_cae
#### # If loaded occurences in TSC were to mismatch original occurrences from TPFL:
tec_site_cae_create <- tec_site_cae %>% 
  dplyr::anti_join(tsc_community_sites, by = "source_id")
tec_site_cae_update <- tec_site_cae %>% 
  dplyr::semi_join(tsc_community_sites, by = "source_id")

bdy_cae_create <- tec_bdy_cae %>% 
  dplyr::anti_join(tsc_community_boundaries, by = "source_id")
bdy_cae_update <- tec_bdy_cae %>% 
  dplyr::semi_join(tsc_community_boundaries, by = "source_id")

## Upload the missing tfa_tae to tsc_tae
site_cae_create_res <- site_cae_create %>% tsc_chunk_post(serializer = "occ-community-points", chunksize = 100)
bdy_cae_create_res <- bdy_cae_create %>% tsc_chunk_post(serializer = "occ-community-points", chunksize = 100)
```
...
```{r make_tec_summary}
tec_comms_ccl_unique <- tec_comms_ccl$community %>%
  unique() %>%
  length() 

tsc_ccl_unique <- tsc_ccl_communities$com_label %>%
  unique() %>%
  length()
```
...
TEC has `r nrow(tec_comms_ccl)` conservation listings of `r tec_comms_ccl_unique` communities. 
TSC has `r nrow(tsc_ccl_communities)` community conservation listings of `r tsc_ccl_unique` unique communities.

### `tec_comms_ccl` conservation listings not in `tsc_ccl_communities`
...
tec_comms_ccl %>%
  dplyr::anti_join(tsc_ccl_communities, by = "source_id") %>%
  rt
```

### `tsc_ccl_communities` conservation listings not in `tec_comms_ccl`

The following table shows any conservation listings in TSC which are not in TEC.

```{r show_tsc_ccl_not_in_tec}
tsc_ccl_communities %>%
  dplyr::anti_join(tec_comms_ccl, by = "source_id") %>%
  rt
```
...
These need to be corrected before the 
...
tsc_listing_cat_wapec <- tsc_ccl_communities %>%
  dplyr::filter(str_detect(category_cache, "WAPEC"))

tsc_listing_cat_dbcarle <- tsc_ccl_communities %>%
...
tsc_cl_sum_wapec <- tsc_ccl_communities %>% 
  make_tsc_cl_summary("WAPEC")
tsc_cl_sum_dbcarle <- tsc_ccl_communities %>% 
  make_tsc_cl_summary("DBCA_RLE")

tec_listing_cat_wapec <- tec_comms_ccl %>%
  dplyr::filter(category %in% c("35", "36", "37", "38"))

tec_listing_cat_dbcarle <- tec_comms_ccl %>%
  dplyr::filter(category %in% c("23", "24", "29", "31", "32", "34")) 
...
tec_listing_cat_wapec %>%
  dplyr::anti_join(tsc_listing_cat_wapec, by = "source_id") %>%
  rt # 106
```
...
tec_listing_cat_dbcarle %>%
  dplyr::anti_join(tsc_listing_cat_dbcarle, by = "source_id") %>%
  rt # 26
```
...
tsc_listing_cat_wapec %>%
  dplyr::anti_join(tec_listing_cat_wapec, by = "source_id") %>%
  rt # 0
```
...
tsc_listing_cat_dbcarle %>%
  dplyr::anti_join(tec_listing_cat_dbcarle, by = "source_id") %>%
  rt # 12
```

### `tec_comms_ccl` unmigrated conservation listings

Below are the listings from `tec_comms_ccl` that could not be assigned to either WAPEC or 
DBCA_RLE and therefore are not migrated, because the `category_code` does not 
correlate to a `list_code`. 

```{r show_tec_ccl_not_migrated}
tec_comms_ccl %>%
  dplyr::anti_join(tec_listing_cat_wapec, by = c("source_id", "effective_from")) %>%
  dplyr::anti_join(tec_listing_cat_dbcarle, by = c("source_id", "effective_from")) %>%
  rt
```
...
-   `r surveys %>% filter(is.na(surveyed_on)) %>% nrow()` boundary 
    survey records lack an explicit date\time and were defaulted to midnight AWST.
-   `r site_visit %>% filter(is.na(site_visit_date)) %>% nrow()` site
    records lack an explicit date\time and were defaulted to midnight AWST.
-   `r tec_bdy %>% dplyr::filter(BDY_ID == "") %>% nrow()` survey records 
    lack an explicit boundary_id so could not be migrated.
-   `r tec_site %>% dplyr::filter(S_ID == "") %>% nrow()` site
    records lack an explicit site_id so could not be migrated.
-   `r tec_bdy_cae_build %>% dplyr::filter(is.na(community)) %>% nrow()` 
    boundary records lack an explicit `community` ID 
-   `r tec_site_cae_build %>% dplyr::filter(is.na(community)) %>% nrow()` 
    site records lack an explicit `community` ID.
-   `r tec_bdy_cae_build %>% dplyr::filter(geom == "null") %>% nrow()` 
    boundary records lack location data (e.g. geometry)
-   `r tec_site_cae_build %>% dplyr::filter(point == "null") %>% nrow()`  
    site records lack location data (e.g. point).
...
```{r show_tec_cae_missing datetime}
# Surveys
...
### Occurrence observations with no matching CAE

Below are occurrence observations which do not have a corresponding CAE boundary 
(table one) or site (table two) for them to be assigned to. 

```{r}
missing_bdy_cae_hab_comp <- tec_hab_comp %>% 
  dplyr::anti_join(tec_bdy_cae, by = "source_id")

missing_site_cae_hab_comp <- tec_hab_comp %>% 
  dplyr::anti_join(tec_site_cae, by = "source_id")

missing_bdy_cae_hab_comp %>% rt
missing_site_cae_hab_comp %>% rt
```

### `tec_cae` occurrence observations not in `tsc_communities_occ_obs`

Any record differences in the following table indicate occurrence observations in TFL that
were not migrated into TSC.

```{r}
make_tec_cae_occ_obs_summary <-
  . %>% nrow() %>% as.data.frame() %>% magrittr::set_colnames("TEC")

tec_hab_comp_sum <-
  tec_hab_comp %>% make_tec_cae_occ_obs_summary
...
tsc_hab_comp_sum <-
  tsc_communities_occ_obs %>% make_tsc_occ_obs_summary("HabitatComposition")


tec_occ_obs_sum <- rbind(
  tec_hab_comp_sum
)
...
occ_obs_summary <- tec_occ_obs_sum %>%
  cbind(tsc_occ_obs_sum) %>%
  cbind(Obstype = c("HabitatComposition")) %>%
...
```{r show individual occurrence observation comparisons}
tec_comms_hab_comp <- tec_hab_comp %>% 
  filter(source_id %in% c("5381", "201", "1821")) %>%
  transmute(
    source_id = source_id,
    tec_landform = landform,
    tec_drainage = drainage)

tsc_comms_hab_comp <- tsc_communities_occ_obs %>% 
  filter(source_id %in% c("5381", "201", "1821")) %>%
  transmute(
    source_id = source_id,
    tsc_criteria = landform %>% as.character(),
    drainage = drainage) %>% 
  dplyr::left_join(tec_landforms, by = "tsc_criteria") %>% 
  select(c("source_id","tsc_landform","drainage")) %>% 
  transmute(
    source_id = source_id,
    tsc_landform = tsc_landform,
    tsc_criteria = drainage %>% as.character()) %>% 
  dplyr::left_join(tec_drainage, by = "tsc_criteria") %>% 
  select(c("source_id","tsc_landform","tsc_drainage")) %>%
  distinct()

tec_hab_comp_sum <- tec_comms_hab_comp %>% 
  left_join(tsc_comms_hab_comp, by = "source_id")
...
### `tec_bdy_cae` occurrences not in `tsc_community_boundaries`

Any records in the following table indicate community occurrences in `tec_bdy_cae` that are not 
...
There are `r nrow(missing_occ_tsc_community_boundaries)` occurrences missing from `tec_bdy_cae` when compared to `tsc_community_boundaries`. 

These missing occurrences come from `r nrow(missing_occ_tsc_community_boundaries_unique)` unique communities. 

The table below is a tally of mismatching boundary occurrences by community.

```{r make_tally_tec_bdy_cae}
tec_bdy_cae_tally <- tec_bdy_cae %>%
  dplyr::group_by(community) %>%
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>%
  ungroup()

tsc_bdy_cae_tally <- tsc_community_boundaries %>%
  dplyr::group_by(community) %>%
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>%
  ungroup()
...
tec_bdy_cae_tally %>%
  dplyr::rename(tec_bdy = n) %>%
  left_join(tsc_bdy_cae_tally, by = ("community")) %>%
  dplyr::rename(tsc_bdy = n) %>%
  filter(tec_bdy != tsc_bdy) %>% rt
```

### `tec_site_cae` occurrences not in `tsc_community_sites`

Any records in the following table indicate occurrences in `tec_site_cae` that are not 
in `tsc_community_sites`.

```{r show_tec_site_cae_missing_occurrences}
...
missing from `tec_site_cae` when compared to `tsc_community_sites`. 

These missing occurrences come from `r nrow(missing_occ_tsc_community_sites_unique)` unique
communities.

The table below is a tally of mismatching site occurrences by community.

```{r make_tally_tfa_tae_tsc}
tec_site_cae_tally <- tec_site_cae %>%
  dplyr::group_by(community) %>%
  dplyr::tally() %>%
  dplyr::arrange(desc(n)) %>%
  ungroup()
...
tec_site_cae_tally %>%
  dplyr::rename(tec_site = n) %>%
  left_join(tsc_site_cae_tally, by = ("community")) %>%
  dplyr::rename(tsc_site = n) %>%
  filter(tec_site != tsc_site) %>% rt
```

# Upload to data catalogue

The [compiled version of this workbook](https://data.dpaw.wa.gov.au/dataset/7942b979-f768-4365-8303-f1b1e00b3a61/resource/7942b979-f768-4365-8303-f1b1e00b3a61/download/eda_communities.html) is uploaded to the 
[Threatened Ecological Communities Dataset](https://data.dbca.wa.gov.au/dataset/threatened-ecological-communities-database) 
...
# TODO add file number from data catalogue
# # Manual mapping of conservation criteria
# upload_file_to_ckan("",
#                     here::here("tec_conscrit_w_tsc.csv"))

# Legacy TEC data
d <- ckanr::package_show("threatened-ecological-communities-database")
upload_to_ckan(
  communities, "Threatened Ecological Communities (TEC)", d$id,
