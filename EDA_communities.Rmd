...
bdy_w_one_site <- tec_bdy %>% 
  filter(S_ID_COUNT == 1) %>% magrittr::extract2("BDY_ID") #121578 with a site ID = 1, which measn they are duplicates 
...
```{r tec_bdy}
tec_bdy %>% filter(BDY_ID == 0) %>% nrow() # Six boundaries have no boundary id
...
setdiff(survey_cae$com_id,communities$com_id) # There are 145 communities not in surveys
...
tec_bdy_cae <- survey_cae %>% 
  dplyr::transmute(
    ...
    description = ifelse(
        is.na(FIRST_S_ID),
        glue::glue("Boundary ID {boundary_id} named ",
...
tec_sit %>% filter(BDY_ID == 0) %>% nrow() #1666
