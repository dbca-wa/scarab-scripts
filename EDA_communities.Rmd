...
```{r make_tec_summary}
tec_ccl_unique <- tec_ccl$com_label %>%
  unique() %>%
  length() 
...
TEC has `r nrow(tec_ccl)` conservation listings of `r tec_ccl_unique` communities. 
TSC has `r nrow(tsc_ccl_communities)` community conservation listings of `r tsc_ccl_unique` unique communities.

### `tec_ccl` conservation listings not in `tsc_ccl_communities`
...
tsc_ccl_communities %>%
  dplyr::anti_join(tec_ccl, by = "source_id") %>%
  rt
```
...
### `tec_ccl` unmigrated conservation listings

Below are the listings from `tec_ccl` that could not be assigned to either WAPEC or 
DBCA_RLE and therefore are not migrated, becasue the `category_code` does not 
