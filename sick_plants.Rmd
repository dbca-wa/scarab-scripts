...
ruODK::ru_setup(
  svc=paste0("https://odkcentral.dbca.wa.gov.au/v1/projects/27/forms/build_Sick-Plants-0-1_1580445581.svc"))
tz <- "Australia/Perth"
loc <- fs::path("data", "media")
fs::dir_create(loc)
```
...
data <- ruODK::odata_submission_get(
  table = form_tables$url[1], wkt=T, verbose = TRUE, tz = tz, local_dir = loc
) 

# data_sickdet <- ruODK::odata_submission_get(
...
data %>% reactable::reactable(filter=T, search=T)
```
...
leaflet::leaflet(width = 800, height = 600) %>%
  ...
  leaflet::addAwesomeMarkers(
    data = data,
    lng = ~details_location_longitude, lat = ~details_location_latitude,
    icon = leaflet::makeAwesomeIcon(text = "Q", markerColor = "red"),
    # 
    # # With your own field names
    # 
    label = ~ glue::glue("{details_species} {observation_start_time}"),
    popup = ~ glue::glue(
      "<h3>{details_species}</h3>",
      "Survey start {observation_start_time}</br>",
      "Reporter {reporter}</br>",
      "Device {device_id}</br>",
      "{details_primary_sickness_description}</br>",
      '<div><img src="{details_habitat_photo}"',
      ' height="150px" alt="Habitat"></img></div>',
      '<div><img src="{details_symptom_photo}"',
      ' height="150px" alt="Symptom"></img></div>'
    ),
