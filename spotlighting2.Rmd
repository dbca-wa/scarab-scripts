...
# remotes::install_github("HanOostdijk/odataR",build_vignettes = T)
library(ckanr)
...
In the code block, some helper functions to wrangle the OData service are defined
and documented.

```{r, message=F}
# OData::metadata(glue::glue("{sss_url}$metadata"))
# odataR::odataR_set_root("https://sandbox.central.opendatakit.org/v1")
# odataR::odataR_get_cat()
...
sss_metadata <- get_metadata(sss_url)
sss <- get_submissions(sss_url)
...
sse_metadata <- get_metadata(sse_url)
sse <- get_submissions(sse_url)
...
spt_metadata <- get_metadata(spt_url)
spt <- get_submissions(spt_url)
...
manually reference each variable any longer.


The metadata for one service endpoint looks like this:
```{r}
sss_metadata %>% listviewer::jsonedit()
```

We don't do anything with the metadata yet.
