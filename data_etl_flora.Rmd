...
flora_cons_listing[1:100, ] %>% DT::datatable(.)
readr::write_csv(flora_cons_listing, path = here::here("data","flora_cons_listing.csv"))
...
setdiff(flora_cons_listing$criteria_code_wa,tfl_conscrit$criteria_code_wa) 
setdiff(tfl_conscrit$criteria_code_wa,flora_cons_listing$criteria_code_wa)
...
tfl_cl_priority <- flora_cons_listing_w_tsc %>% 
...
tfl_cl_priority %>% 
  wastdr::wastd_POST("taxon-conservationlisting", 
                     api_url = uat, verbose=TRUE)

# Active Priority listings:
tfl_cl_priority %>% dplyr::filter(status==80) %>% nrow

# Inactive Priority listings:
tfl_cl_priority %>% dplyr::filter(status==90) %>% nrow
...
tfl_cl_epbc <- flora_cons_listing_w_tsc %>% 
...
tfl_cl_iucn1994 <- flora_cons_listing_w_tsc %>% 
...
tfl_cl_iucn2001 <- flora_cons_listing_w_tsc %>% 
...
tfl_cl_iucn2012 <- flora_cons_listing_w_tsc %>% 
