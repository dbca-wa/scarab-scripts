...
tfl_listing_cat_priority %>% 
  chunk_post(serializer = "taxon-conservationlisting", chunksize = 100)
...
There are `r tfl_cl_priority %>% dplyr::filter(status==80) %>% nrow` active Priority listings in WA and
`r tfl_cl_priority %>% dplyr::filter(status==90) %>% nrow`
