...
gjlocal <- function(data, ser) upsert_geojson(data, serializer = ser, verbose = T, api_url = local_api)
"public:herbie_hbvnames_public" %>% gs_getFeature() %>% gjlocal("names")
"public:herbie_hbvsupra_public" %>% gs_getFeature() %>% gjlocal("supra")
"public:herbie_hbvgroups_public" %>% gs_getFeature() %>% gjlocal("groups")
...
"9782cc52-7be8-494a-a7da-51845c119a21" %>%
  load_ckan_csv() %>%
  transmute(
    name_id = com_id + 1000000, # hack!
    name = paste0("(", com_label, ") ", com_name), # hack!
    rank = 7, # brittle!
    parent = 39480, # brittle!
    current = TRUE
  ) %>%
  wastd_POST("taxon")
```
