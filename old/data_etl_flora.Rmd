...
flora_data production data lives in an Oracle database. An MS Access database (510 MB) is generated from 
...
# Data
The zipped MS Access snapshot is downloaded, unpacked, and accessed as object `flora_data`.

```{r flora_load_data}
flora_data <- dl_mdbzip("77062fa0-f409-4fe7-ae28-de8c933a59bd")
...
leaflet(width = 800, height = 600) %>%
  ...
  clearBounds() %>%
  addAwesomeMarkers(
    data = flora_data$DRF_POPULATION,
    lng = ~ GDA94LONG, lat = ~ GDA94LAT,
    icon = leaflet::makeAwesomeIcon(icon = "leaf"),
    label = ~ paste(TAXONID),
    popup = ~ glue::glue(
      '
      <h3>Taxon {TAXONID} Pop {POP.ID}</h3>
      <strong>Encountered on</strong> {COUNT.SECT.DATE}<br/>
      {HABITAT.CONDITION}{HABITAT.NOTES}<br/>
      {LOCATION}
      '
    ),
    group = "Flora",
...
```{r pop}
populations <- flora_data$DRF_POPULATION %>%
  dplyr::transmute(
...
flora_occ <- flora_data$DRF_RFR_FORMS %>%
  dplyr::transmute(
    # IDs
...
leaflet(width = 800, height = 600) %>%
  ...
  addAwesomeMarkers(
    ...
    popup = ~ glue::glue(
      ...
      Sheet no {sheetno}. New population: {new_pop_ind}. {population_notes}<br/>
...
```{r occ_vegclass}
occ_vegclass <- flora_data$DRF_SHEET_VEG_CLASSES %>%
  dplyr::transmute(
...
```{r occ_threats}
occ_threats <- flora_data$DRF_SHEET_THREATS %>%
  dplyr::transmute(
...
```{r occ_fire}
occ_fire <- flora_data$DRF_FIRE_DETAILS %>%
  dplyr::transmute(
...
```{r pop_forms}
pop_forms <- flora_data$DRF_POP_FORMS %>%
  dplyr::transmute(
...
```{r pop_threats}
pop_threats <- flora_data$DRF_POP_THREATS %>%
  dplyr::transmute(
...
```{r pop_mgmtactions}
pop_mgmtactions <- flora_data$DRF_POP_MGMT_ACTIONS %>%
  dplyr::transmute(
...
```{r pop_fire_summ}
pop_fire_summ <- flora_data$DRF_FIRE_SUMMARIES %>%
  dplyr::transmute(
...
```{r liaisons}
liaisons <- flora_data$DRF_LIAISONS %>%
  dplyr::transmute(
...
```{r pop_vegclass}
pop_vegclass <- flora_data$DRF_POP_VEG_CLASSES %>%
  dplyr::transmute(
...
## Metadata indicating what information (SECT_CODE) is available for each population.

```{r pop_sectionmap}
pop_sectionmap <- flora_data$DRF_POP_SECTION_MAP %>%
  dplyr::transmute(
...
```{r conslist}
conslist <- wastdr::wastd_GET("conservationlist") %>% 
  wastdr::wastd_parse() %>% 
  dplyr::select(
...
```{r conscat}
conscat <- wastdr::wastd_GET("conservationcategory") %>% 
  wastdr::wastd_parse() %>% 
...
readr::write_csv(conscrit, path = here::here("data/tsc_conscrit_flora.csv"))
```

## TFL conservation Listing

```{r tfl cons_listings}
flora_cons_listing <- flora_data$DRF_TAXON_CONSV_LISTINGS %>%
  dplyr::transmute(
...
flora_cons_listing %>% rt
readr::write_csv(flora_cons_listing, 
                 path = here::here("data","flora_cons_listing.csv"))

flcl_file <- here::here("data", "flora_cons_listings.Rdata")
save(flora_cons_listing, file=flcl_file)
if (file.exists(flcl_file)) load(flcl_file)
```
Conservation lists used in Flora cons listings:

```{r tfa_cons_listing_qa}
# ETL nightmare fuel: condensed criteria mixed with free text

## Find all criteria combinations for manually mapping
all_crit_map <- flora_cons_listing %>%
...
```{r compare_tsc_conscrit_listing_to_tfl_conscrit}
...
conscrit_not_in_mapping <- base::setdiff(
  flora_cons_listing$criteria_code_wa, tfl_conscrit$criteria_code_wa)

if(conscrit_not_in_mapping != "")
  wastdr::wastdr_msg_warn(glue::glue(
    "Cons criteria in data but not in mapping: {conscrit_not_in_mapping}"))

conscrit_not_in_data <- base::setdiff(
  tfl_conscrit$criteria_code_wa, flora_cons_listing$criteria_code_wa)

if(length(conscrit_not_in_data) > 1)
  wastdr::wastdr_msg_warn(glue::glue(
    "Conservation criteria in mapping but not in data: {conscrit_not_in_data}"))

flora_cons_listing_w_tsc <- flora_cons_listing %>% 
  dplyr::left_join(tfl_conscrit, by=c("list_code_wa", "criteria_code_wa"))
```

```{r cons_listing_tsc_upload, eval=T}
...
tfl_listing_cat_priority <- flora_cons_listing_w_tsc %>% 
...
tfl_listing_cat_priority %>% 
  chunk_post(serializer = "taxon-conservationlisting", chunksize = 1000)
...
tfl_listing_cat_epbc <- flora_cons_listing_w_tsc %>% 
...
tfl_listing_cat_iucn1994 <- flora_cons_listing_w_tsc %>% 
...
tfl_listing_cat_iucn2001 <- flora_cons_listing_w_tsc %>% 
...
tfl_listing_cat_iucn2012 <- flora_cons_listing_w_tsc %>% 
...
# List codes: "FLORABASE"
# TSC will export TO FloraBase, not the other way around
```

There are `r tfl_listing_cat_priority %>% dplyr::filter(status==80) %>% nrow` 
active Priority listings in WA and
`r tfl_listing_cat_priority %>% dplyr::filter(status==90) %>% nrow`
delisted Priority listings in WA.
...
```{r cons_prio}
cons_prio <- flora_data$DRF_P2K_PRIORITY %>%
  dplyr::transmute(
...
if (fs::file_exists("data_etl_flora.html")) ckanr::resource_update(
    "d1fccebb-02e2-4a40-aba4-c4d6ad012edb", "data_etl_flora.html")
