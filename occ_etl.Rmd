...
```{r helpers}
#' Return the convex hull around all given points.
#'
...
anim_24557 <- fauna_occ %>% 
    dplyr::mutate(year = lubridate::year(datetime)) %>% 
    dplyr::filter(name_id==24557)

oz <- ggplot() +
  borders("world", colour = "gray75", fill = "gray95") +
  ggthemes::theme_map() + coord_equal() + xlim(c(100, 140)) + ylim(c(-40,-10))

oz + geom_point(
        data=anim_24557,
        aes(x=longitude, y=latitude, size = number_seen, colour = observation_type), 
        alpha = 0.7, show.legend = FALSE) +
  scale_size(range = c(2, 12)) +
  labs(title = 'Mallefowl [Year: {lubridate::year(frame_time)}]', x = 'Lat', y = 'Lon') +
  transition_time(o_date) +
  # gganimate::shadow_mark(size = 1, colour = 'darkgrey') +
  ease_aes('linear') +
  enter_grow() +
  exit_fade() +
  anim_save("data/occ_24557.gif")
```
