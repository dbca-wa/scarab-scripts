...
fauna_tae <- fauna_occ %>%
  dplyr::filter(!is.na(name_id) & !is.na(latitude) & !is.na(longitude)) %>% 
  dplyr::transmute(
    taxon = name_id,
    code = location_name,
    name = location_name,
...
fauna_tae10 <- fauna_tae %>% dplyr::filter(taxon==24734)
fauna_tae10 <- fauna_tae %>% dplyr::filter(source_id==10261)
fauna_tae %>% chunk_post(serializer = "occ-taxon-points", verbose = T, api_url = prod)
```
...
flora_tae %>% chunk_post(serializer = "occ-taxon-points", verbose = T, api_url = prod)
```

## TEC
```{r tec_occ_tsc , eval=F}
tec_boundaries <- geojsonsf::geojson_sf("data/tec_boundaries.geojson")
tec_cae <- tibble::tibble(
    community = tec_boundaries$COM_ID,
    name=tec_boundaries$BDY_ID,
    description = glue::glue(""),
    area_type = 10, # 10 bound, 11 buffer, 12 site point
    source = 12, # TEC
    source_id = tec_boundaries$OCC_UNIQUE,
    encountered_on=default_date,
    encountered_by=1,
    geom = tec_boundaries$geometry %>% 
        sf::st_convex_hull(.) %>% 
        sf::st_buffer(., 0.00001) %>% 
        geojsonsf::sfc_geojson(.) 
    )
tec_cae10 = tec_cae[1:100,]
tec_cae %>% chunk_post(serializer = "occ-community-areas", api_url = dev, verbose = T)
#"http://0.0.0.0:32769/api/1/")
```
...
Serial offenders:
```{r tec_cae_large}
tec_cae %>% dplyr::group_by(community) %>% dplyr::tally() %>% dplyr::arrange(-n) %>% filter(n>400)
```
...
```{r}
fcl_file <- here::here("data", "fauna_cons_listing.Rdata")
if (file.exists(fcl_file)) load(fcl_file)
```

Conservation lists used in Fauna cons listings:
```{r}
fauna_cons_listing$list_code %>% unique # cons lists used
fauna_cons_listing %>% dplyr::filter(list_code == "WAPF") # Is this WAPS?
fauna_cons_listing %>% dplyr::filter(list_code == "ActionPlan") # Discard those?
fauna_cons_listing %>% dplyr::filter(list_code == "") # Discard those?
fauna_cons_listing %>% dplyr::filter(list_code == "RedList") # Which IUCN version?

# ETL nightmare fuel: condensed criteria mixed with free text
criteria_requiring_mapping <- fauna_cons_listing %>% 
    dplyr::filter(grepl('[+,;]', reasons)) 

criteria_requiring_mapping %>%  unique %>% knitr::kable()
```

Questions:

* Fauna WAPF = TSC WAPS? (Has same codes, so assume yes)
...
`r criteria_requiring_mapping %>% nrow` conservation listings contain condensed lists of criteria
mixed with free text. Rather than manually transcribing this number, a manually written mapping for
`r criteria_requiring_mapping$reasons %>% unique %>% length` distinct values can be used to automate ETL.

### Flora cons listing
```{r}
tflcl_file <- here::here("data", "flora_cons_listings.Rdata")
if (file.exists(tflcl_file)) load(tflcl_file)
```

```{r}
flora_cons_listings %>% DT::datatable()
```
