...
flora_tae %>% dplyr::group_by(taxon, name) %>% dplyr::tally() %>% dplyr::filter(n>2)
```

Finally, flora encounters are uploaded to TSC.
```{r flora_occ_tsc_upload, eval=F}
flora_tae10 = flora_tae[1:100,]
flora_tae10 %>% chunk_post(serializer = "occ-taxon-points", verbose = T, api_url = dev)
```
