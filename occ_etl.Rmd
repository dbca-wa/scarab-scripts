...
wastdr::wastd_POST(fauna_tae, serializer = "occ-taxon-points", api_url = dev)
```
## Flora

```{r flora_occ_tsc, eval=F}
flora_tae <- flora_occ %>%
  dplyr::filter(!is.na(name_id)) %>% 
...
wastdr::wastd_POST(flora_tae, serializer = "occ-taxon-points", api_url = dev)
```
...
tec_cae %>% chunk_post(serializer = "occ-community-areas", api_url = uat)
```
